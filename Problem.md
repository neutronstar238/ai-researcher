...
- Root cause: The MVP originally used tiny synthetic ScientistBench-Lite fixtures; task `63.1` added a real benchmark path, but the full system still lacks robust multi-source retrieval coverage, stronger candidate-specific query generation, paper-structured drafting, and method novelty beyond a baseline benchmark.
...
- Next action: Re-run the full publication audit after OpenAlex is available in the default loop, improve similarity query generation so four distinct project-start cross-searches are issued, generate paper-structured drafts only after sufficient evidence, and add stronger method novelty beyond the Pendigits baseline.
- Linked tasks: `61.1`, `63.1`, `64.1`
