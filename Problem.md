...
- Next action: Task `268.2` must be re-scoped before implementation. Its current premise, "fix only the shape-transport defects found in `268.1`", does not match the evidence, because there is no shape-transport defect to fix. A legitimate repair must instead choose explicitly between (a) aligning our runner's error handling with MDBench's own harness so a library-side complexity failure degrades rather than discarding the fit, and (b) recording a baseline erratum that admits the pinned library cannot evaluate a 3-sample axis at `derivative_order=2`. Both routes must preserve the frozen numeric grid, and any change that would weaken the baseline is a gate violation rather than a repair. Escalate the re-scope to the user before writing code.
- Linked tasks: `268.1`, `268.2`, `268.3`, `268.4`, `266.1`, `269.4`.
- Resolution: Not resolved. Task `268.1` is diagnosis only; the fault-ownership classification is the deliverable.
