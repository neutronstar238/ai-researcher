...
- Root cause: The MVP originally used tiny synthetic ScientistBench-Lite fixtures; task `63.1` added a real benchmark path, task `64.1` added OpenAlex source fallback, task `65.1` fixed sparse query breadth, task `67.1` aligned the default runtime with publication-width search, and task `69.1` added paper-structured Markdown drafting. The full system still lacks Semantic Scholar API stability, LaTeX template compatibility, and method novelty beyond a baseline benchmark.
...
- Next action: Stabilize or key Semantic Scholar access, decide how source-error severity should interact with OpenAlex/ArXiv fallback, add LaTeX template compatibility testing, and add stronger method novelty beyond the Pendigits baseline.
