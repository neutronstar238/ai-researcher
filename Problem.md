...
- Root cause: The MVP originally used tiny synthetic ScientistBench-Lite fixtures; task `63.1` added a real benchmark path, task `64.1` added OpenAlex source fallback, task `65.1` fixed sparse query breadth, task `67.1` aligned the default runtime with publication-width search, task `69.1` added paper-structured Markdown drafting, and task `70.1` added generic LaTeX PDF compatibility smoke. The full system still lacks Semantic Scholar API stability, external venue-template compatibility, and method novelty beyond a baseline benchmark.
...
- Next action: Stabilize or key Semantic Scholar access, decide how source-error severity should interact with OpenAlex/ArXiv fallback, expand LaTeX compatibility to external official/canonical templates, and add stronger method novelty beyond the Pendigits baseline.
- Linked tasks: `61.1`, `63.1`, `64.1`, `65.1`, `67.1`, `69.1`, `70.1`
