...
- Workaround: Keep Task `260` v2 immutable and additive. Task `263.7.1` now supplies the truthful task-level correction and retires the old publication inference without replacing the preregistration. Complete Tasks `263.7.2` and `263.7.3` for current-field positioning and RO-Crate/PROV interoperability while keeping publication claims blocked.
...
- Resolution: The seed-pseudoreplication risk is now mitigated by a fail-closed audit plus a separately manifested task-unit correction, but stale positioning, external-evidence, task-family, scorer, baseline, and human-review gaps remain open. Publication, public release, and external submission remain false.
- Verification: Eight deterministic Task `263.7.0` tests and eight Task `263.7.1` tests pass. Both opt-in live smokes validate the immutable real parent and exact replay in two clean Python installations; the newer smoke also validates complete claim/number/table/surface binding, recursive persistence, and parent/audit parity. Broader repository verification is recorded in `Agent.md`.
