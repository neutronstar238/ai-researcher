...
- Root cause: The MVP originally used tiny synthetic ScientistBench-Lite fixtures; task `63.1` added a real benchmark path, task `64.1` added OpenAlex source fallback, task `65.1` fixed sparse query breadth, and task `67.1` aligned the default runtime with publication-width search. The full system still lacks Semantic Scholar API stability, paper-structured drafting, and method novelty beyond a baseline benchmark.
...
- Next action: Stabilize or key Semantic Scholar access, decide how source-error severity should interact with OpenAlex/ArXiv fallback, generate paper-structured drafts only after sufficient evidence, and add stronger method novelty beyond the Pendigits baseline.
- Linked tasks: `61.1`, `63.1`, `64.1`, `65.1`, `67.1`
