...
- `airesearcher code-agents opencode init|list` and `integrations/opencode/code-agent.json` for a preferred direct OpenCode external code-agent contract using OpenCode `run`, `serve`, ACP, permissions, project commands, and project skills while AI-Researcher keeps validation, approval, merge, rollback, and Obsidian logging authority.
...
- Publication-audit method innovation gate for CCF-B/Q3-style targets, requiring proposed mechanism/contribution metadata plus a file-backed innovation/mechanism artifact before baseline results can be treated as paper-level research.
- Publication-audit method-effect gate for CCF-B/Q3-style targets, requiring file-backed innovation artifacts to report a positive baseline-vs-candidate effect delta before empirical-gain claims can pass.
- Publication-audit method-effect gate now rejects weak positive CCF-B/Q3 claims when the recorded delta is smaller than 2.0 standard errors; real Spambase verification completes the loop but blocks release at 0.76 standard errors.
...
- Opt-in `skin_variance_calibrated_prototypes` public benchmark demo that downloads the whitespace-delimited UCI Skin Segmentation source file at run time, compares a z-score nearest-centroid RGB baseline against diagonal variance-calibrated skin/non-skin prototypes, and records a strong source-backed method effect for stability-matrix cycles.
...
- Project-start similarity checks now classify source-backed skin detection, skin-color classifier, RGB color-model, and skin-image segmentation prior work while keeping broad skin-color social-media usage and unrelated segmentation tasks `unknown`.
...
- LLM evidence review now keeps a larger final-manuscript subject excerpt while shortening per-evidence excerpts, reducing empty or truncated provider responses during full-paper review.
