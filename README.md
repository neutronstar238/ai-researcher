...
AI-Researcher is an evidence-first platform for automated computational research. It is built to run as a local or server-side research operator: discover sources, maintain an Obsidian knowledge vault, propose hypotheses, execute reproducible experiments, validate results, draft LaTeX papers, and feed failures back into a controlled self-evolution loop.

> Status: local MVP with tested research-loop components. The repository includes the executable task plan, Obsidian vault substrate, provider-agnostic deployment setup, always-on CLI entrypoints, public-source literature/similarity retrieval, real benchmark demos, validation/reporting modules, LaTeX paper builds, OpenCode integration contracts, and release-preparation gates. It is not yet a production multi-user service, and it must not claim CCF-B/Q3 publishability unless the evidence gates pass.

## Why This Exists

Most automated research demos start with writing. AI-Researcher starts with evidence. A result should only become a claim when the system can trace it back to a real run, configuration, metric file, source artifact, and validation status.
...
The first usable milestone is not a fully autonomous scientist. It is a trusted research loop that can stay online, pull public research signals, run a bounded computational experiment, collect outputs, validate them, write evidence back into Obsidian, and refuse to release weak paper claims.
...
- ArXiv and OpenAlex are the default core academic sources. Semantic Scholar is an optional enhancement source because unauthenticated Graph API access can hit 429 limits; enable it only when you have a key or explicitly want the extra metadata.
...
If you prefer to fill the model configuration manually, copy `.env.example` to `.env` and set `AUTORESEARCH_LLM_BASE_URL`, `AUTORESEARCH_LLM_MODEL_NAME`, and `AUTORESEARCH_LLM_API_KEY`. AI-Researcher uses ArXiv and OpenAlex as its default free/public academic sources. OpenAlex can run without a key at small scale; optional `OPENALEX_API_KEY`, `OPENALEX_MAILTO`, `OPENALEX_MIN_INTERVAL_SECONDS`, and `OPENALEX_CIRCUIT_RESET_SECONDS` settings make larger deployments more polite and predictable. Semantic Scholar is optional: set `AUTORESEARCH_ENABLE_SEMANTIC_SCHOLAR=1` or provide `SEMANTIC_SCHOLAR_API_KEY` only when you want it as an extra metadata source, plus optional `SEMANTIC_SCHOLAR_MIN_INTERVAL_SECONDS` and `SEMANTIC_SCHOLAR_CIRCUIT_RESET_SECONDS` values for stricter throttling. The root `.env` file is intentionally ignored by git and must never be committed.
...
Within one `autopilot` or `serve` cycle, literature refresh and similarity checking share the same source clients. The default client set is ArXiv plus OpenAlex. If you explicitly enable Semantic Scholar and it opens a 429 circuit, the similarity phase inherits that circuit-open state instead of rebuilding a fresh client and hammering the same source again. The circuit state is also persisted under the selected literature cache root as `source-circuit-breakers.json`, so a later cycle in the same deployment can respect the cooldown window before trying the source again. That state file is written through a same-directory temporary file and atomic replace, so an interrupted write should not leave a half-written cooldown file; read-modify-write updates also use a local `.lock` file so concurrent workers do not silently overwrite each other's source cooldowns. Before costly work starts, a SCALE-lite source preflight gate reads persisted source state without making network calls. Required source cooldown/state failures stop the cycle before experiment, review, and paper-build work; optional Semantic Scholar cooldowns are recorded as degraded optional coverage and do not block the core free-source loop.
...
The literature and similarity commands use real literature APIs by default: ArXiv and OpenAlex. Semantic Scholar is available as an opt-in enhancement source, not a required dependency. The commands load optional literature API keys from `.env`, apply conservative source-specific rate limiting with tunable request spacing and 429 circuit breaking, preserve per-source fetch errors, and write guarded Obsidian summaries that keep unsupported outcomes as `unknown` or `pending verification`. Similarity summaries can classify direct duplicates, adjacent work, supporting prior work, contradictory evidence, and benchmark gaps, but only when source title/abstract metadata supports the classification; conservative method/dataset token overlap is recorded in the classification basis, and weak live hits remain `unknown`. The project-start query generator now prefers short structured query forms over long prose for the first publication-gate searches, while keeping research-gap, negative-result, and vault-context queries available as breadth. OpenAlex is part of the core free-source path so Semantic Scholar 429s do not collapse discovery coverage.

`inspiration-refresh` calls real broad web-adjacent sources that are not academic databases. The current built-in sources are Hugging Face public dataset metadata and Hacker News Search. Results are useful for dataset leads, tool ideas, engineering constraints, community pain points, and adjacent project discovery, but the generated Obsidian note labels them as dataset/community/news signals only. They do not count as scholarly evidence, novelty evidence, or publication-gate support until a later step validates them against primary sources, code, data cards, or executed experiments.
