...
- [ ] 263. Recover publication-grade scientific search through replication-first portfolios
  ...
  - [ ] 263.6 Execute untouched confirmatory evaluation and independent adjudication
    ...
    - [ ] 263.6.7 Freeze and execute an AI-scientist benchmark-validity systematic mapping study
      ...
      - [ ] 263.6.7.2.1 Freeze the pre-extraction API-pagination erratum
        - Preserve Task `263.6.7.1` as immutable history, but bind an additive signed erratum to its protocol hash before any formal query: Crossref must start cursor paging with `cursor=*` and stop on a short page because current official documentation returns `next-cursor` even on the last page; DBLP must bind the exact 2023/2024/2025/2026 year-qualified fallback queries and stop partial if the 1,000-hit cap is reached. Reconfirm OpenAlex's initial cursor and source-specific parameter spellings without inspecting benchmark outcomes.
