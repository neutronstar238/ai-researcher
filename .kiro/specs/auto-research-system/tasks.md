...
- [x] 260. Autonomous CCF-B contribution campaign
  ...
  - [x] 260.3 Connect local Ollama and real benchmark adapters, then run two new rounds
    ...
    - _Outcome: the metadata-only audit found 49 unused ODE and 8 unused PDE systems after excluding both earlier official panels, then froze two disjoint six-system ODE holdouts. Campaign `task260-autonomous-ccfb-v1` completed two new 240-cell rounds with zero research-decision human interventions. Noise-conditioned ensemble SINDy passed development at `0.779785` median relative improvement but failed its frozen unseen gate with system-level 95% CI `[-3.053723, 0.953866]`; spline/group-sparse SINDy passed development at `0.672083` but failed unseen with `[-2.157336, 0.921594]`. Both rounds are retained as hash-bound negative results, terminal resume is idempotent, and Route A stopped without changing the revealed panels or thresholds._
    - _Verify: four new adapter/audit tests, 15 focused campaign tests, full pytest/Ruff/Mypy, Docker image build and two-mechanism self-test, official-archive audit, live local Ollama compatibility smoke, two official SNR20 cells, the real two-round campaign, terminal status/resume, 1,289-file export integrity, and `git diff --check`; exact commands and evidence hashes are recorded in `Agent.md`._
  - [x] 260.4 Execute the autonomous-research systems-paper matrix when Route A cannot pass
...
- [x] 261. Close the autonomous-origin and manuscript-evidence gaps
  ...
  - [x] 261.2 Generate and test a new mechanism from the retained negative result
    ...
    - [x] 261.2.2 Generate, review, and development-screen one executable mechanism
      ...
      - _Outcome: local `qwen3.5-sprint:9b-8k` produced a parent-bound diagnosis and the `Multi-Signal Degradation Gate with Source Counting` structured mechanism program. A versioned trusted compiler supplied only the fixed non-scientific JSONL loop while the model-owned risk/accept expressions and reason codes determined behavior; no code-side repair or mechanism fallback was used. The exact generated source passed static review, deterministic unit/property probes, secret/network/path guards, and bounded Harness execution before the three-task development partition was opened. It accepted 18/24 claims with coverage `0.75` and unsupported-accept rate `0.0`, so the development-only decision is `advance_to_preregistration`; six confirmatory tasks remain sealed, no confirmatory result artifact exists, no scientific result was created, and external submission remains false. The authoritative v12 manifest hash is `55c4604474517317114fa88fa389aced28ca5ba96f2eafee6832cfcceb24737e`._
      - _Verify: 35 focused tests passed with the opt-in smoke skipped; the separately enabled real local-model smoke passed once against `qwen3.5-sprint:9b-8k`; adversarial expression/source tests, exact-byte tamper checks, closed numeric-boundary probes, generated-code evidence, and Harness episode hashes passed; repository-wide pytest/Ruff/Mypy, Vault-link checks, CLI status validation, and `git diff --check` passed. Failed/superseded v1-v11 attempts and the post-audit v13 property-blocked diagnostic remain retained and are not completion evidence; the frozen v12 source separately passed the final dual-entrypoint preflight and Harness replay._
    - [x] 261.2.3 Freeze and adjudicate a new independent confirmatory panel
      ...
      - _Outcome: froze v12 source `7b4961c62a7b8a253eb44d1e656dde3abc30dc1d6c1fc4e25b17745eca137025`, environment `0198b9e7a8c13258d139ce4398162c6c272c491aa64ff3358aa63a06a67b1ea8`, immutable Control Graph `fe2d9e96b264d86b5ae87602dce4628c72de49019d17a48344cba8051b7fab44`, six source-fingerprint-disjoint tasks, task-level 20,000-resample bootstrap, minimum coverage `0.60`, maximum unsupported-accept rate `0.10`, and one-attempt stop policy before reveal. The one-shot real run executed all six tasks successfully and retained 20 abstentions; unsupported risk passed at `1/28 = 0.0357` with 95% interval `[0.00, 0.10]`, but coverage was `28/48 = 0.5833` with interval `[0.4792, 0.6875]`, so the immutable scientific endpoint is `negative_result` solely because `minimum_coverage_met=false`. Endpoint hash is `d449343654e28a4da877d0ab7a3bd07e334ac8cad310385996c635bacbae165d`; same-panel tuning, endpoint rewrite, and external submission remain false._
      - _Verify: five deterministic disjoint/tamper/negative/idempotent-resume tests; 96 related Campaign/Harness/Loop/Journal/provenance tests; one separately enabled real confirmatory smoke; six one-attempt task receipts; sealed Event Journal; provenance-v2 claim trace; all-pass evaluation/security report; independently recomputed clean-directory scientific projection; non-destructive pre-reveal rollback rehearsal; 220-file no-change idempotent reload; full regression with 958 tests passed and 16 opt-in tests skipped; repository-wide Ruff, 157-file Mypy, and `git diff --check`._
    - [x] 261.2.4 Build the child paper and claim-evidence audit
      ...
      - _Outcome: built the immutable negative-result child-paper package from the task 261.2.3 endpoint without rerunning or rewriting the six-task panel. The 2,512-word, 13-page manuscript contains 51 registered material paragraphs, 26 typed evidence records, and 77 supporting links; all 51 claims pass exact-occurrence, typed-evidence, and frozen-source entailment checks. All 14 verified sources appear as named-work claims, inline citations, and references across the four frozen evidence areas; a live check returned 14 substantive HTTP 200 responses. Five source-backed figures and one six-task table pass caption, reference, metric, and endpoint-consistency checks. A clean-directory rebuild matched all 24 deterministic manuscript/data/figure/table/audit sources and independently compiled another 13-page quality-passing PDF. Package manifest `462c428dc1c863407042ae48ad1cb2245a942ba0af93744a0022804eeb26bcc8` remains bound to negative endpoint `d449343654e28a4da877d0ab7a3bd07e334ac8cad310385996c635bacbae165d`; scientific submission, authorship, license, explicit human approval, submission readiness, and external submission remain false._
      - _Verify: 17 focused claim/paper/figure/table tests; one separately enabled live-source and dual-PDF smoke; hash-valid CLI status; all 13 PDF pages rendered and visually inspected with no clipping, overlap, unreadable display, or unresolved reference marker; full regression with 983 tests passed and 17 opt-in tests skipped; repository-wide Ruff; Mypy across 158 source files; Vault-link checks and `git diff --check`; and one focused parent-task completion commit._
...
- [x] 262. Unify Graph, Harness, Loop, provenance, and Open Science semantics
  ...
  - [x] 262.2 Add canonical run-event and four-plane graph contracts
    ...
    - _Outcome: added `autoresearch.kernel` with strict v1 `RunEvent`, `EventActor`, `GraphNode`, `GraphEdge`, and `GraphSnapshot` contracts. Events use normalized UTC/JSON content, stable artifact-reference sets, parent/fork rules, idempotency identity, canonical JSON, and a validated SHA-256; graph snapshots normalize node/edge sets and require explicit cycle-boundary semantics for intentional control loops. No legacy service or dependency changed._
    - _Verify: 31 focused unit/property tests reached 100% coverage of `contracts.py`; the full suite passed with 778 passed and 5 opt-in live tests skipped; full Ruff and Poetry-environment Mypy passed; schema/import smoke exported all five public contracts; `git diff --check`, Agent/Problem updates, and one focused commit. Exact commands are recorded in `Agent.md`._
  - [x] 262.3 Add the atomic append-only event journal, replay, and fork
    ...
    - _Outcome: added a directory-backed v1 `EventJournal` with one canonical immutable file per contiguous event, exclusive writer leases, expected-lineage checks, exact idempotent retry, parent/hash-chain validation, run lineage hashes, deterministic terminal seals and recovery, replay/checkpoint, immutable-parent fork anchors, and full-envelope secret/direct-email rejection. Existing service state files and write paths remain authoritative and unchanged._
    - _Verify: 33 focused unit/fault/Hypothesis tests passed with 89% `journal.py` coverage; a real temporary-filesystem create/append/seal/reopen/replay smoke passed; the full suite passed with 811 passed and 5 opt-in live tests skipped; full Ruff and Mypy passed; exact commands are recorded in `Agent.md`._
  - [x] 262.4 Add HarnessSpec, bounded execution policies, and episode packages
    ...
    - _Outcome: added a content-addressed provider-neutral `HarnessSpec`, cross-policy validation, a single-trial bounded `HarnessRunner`, deterministic fixture/grader protocols, content-addressed `EpisodePackage`, and a configurable OpenAI-compatible adapter. Episodes separately bind task/spec, trials, full trajectory, environment outcome, graders, cost, interventions, approvals, failures, tool calls, artifacts, terminal event, journal seal, and lineage. Missing models/tools/approvals and exhausted budgets are blocked; invalid structured output, tool/grader/configuration failures are failed; valid non-passing evaluations remain negative results; no failed path synthesizes scientific output. Existing services and dependencies remain unchanged._
    - _Verify: 31 focused deterministic tests passed with one default-skipped live test; `harness.py` reached 92% and the OpenAI-compatible adapter 98% line coverage. An explicit opt-in live smoke passed against configured local `qwen3.5-sprint:9b-8k` and produced a successful sealed episode without persisting endpoint, credential, or raw model text. The full suite passed with 824 tests and 6 opt-in live tests skipped at 86% repository coverage; full Ruff passed; Mypy passed for 140 source files; exact commands and resolved diagnostics are recorded in `Agent.md` and `Problem.md`._
  - [x] 262.5 Add LoopSpec and a durable Control Graph runtime
    ...
    - _Outcome: added a content-addressed `LoopSpec`, deterministic journal-backed `ControlGraphRuntime`, explicit nodes/edges/guards and bounded retry/approval/compensation/stop/pivot/escalation semantics, holdout isolation, immutable graph proposals, stable node idempotency keys, and content-addressed run snapshots. `EventJournal` remains the sole canonical run state. A thin LangGraph adapter sits behind the domain runtime and freezes installed 0.2.76 checkpoint/resume, static/dynamic interrupt, subgraph, parallel-superstep, idempotency, and JSON-serialization behavior without changing dependency versions. A verified `EpisodePackage` can feed the graph through a provider-neutral result projection; legacy services remain authoritative and unchanged._
    - _Verify: 24 focused lifecycle/property/fault/adapter/vertical tests passed with 86% `loop.py` and 97% LangGraph-adapter line coverage; a 32-test compatibility matrix proved the legacy experiment loop and new Control Graph tests collect together. A persisted development vertical produced independently sealed Harness and Control Graph journals and a successful content-addressed snapshot. The full suite passed with 848 tests and 6 opt-in live tests skipped at 86% coverage; full Ruff and 142-file Mypy passed. Exact commands, hashes, and resolved diagnostics are recorded in `Agent.md`, `Problem.md`, and the task Vault note._
  - [x] 262.6 Add W3C PROV-aligned evidence v2 and source-anchored Vault projections
    ...
    - _Outcome: added content-addressed, provider-neutral provenance-v2 contracts for the W3C PROV starting points and qualified relations, research claims and three evidence directions, validation/revision/invalidation history, decisions, tools, and digest-only model interactions. A strict bundle validates IDs, references, UTC valid time, versions, source snapshots, responsibility, derivations, content hashes, and its own canonical digest. `require_claim_trace()` fails closed unless a core claim reaches verified source/input, generating activity, frozen code/software or policy agent, artifact, current validation, and generated decision artifact. Existing EvidenceGraph v1 code is unchanged and receives a compatibility projection. Vault projection requires an explicit approval allow-list and maps literature, hypotheses, failures, skills, strategies, experiment records, evidence, and decisions to source-anchored Markdown._
    - _Verify result: 9 focused provenance/evidence/Vault unit tests and one deterministic campaign integration passed; a 43-test legacy/new/Vault matrix passed with the real smoke skipped by default. The opt-in smoke validated existing `task260-autonomous-ccfb-v1/round-001`, produced bundle hash `a2e54556b3f6e242deeaff3d7c87400ae23e701ef034983fb6964a3c2df4c782`, wrote 12 approved Vault notes, preserved two v1 claims/three evidence edges, and blocked both nested tampering and removal of the gate generation. The full suite passed with 858 tests and 7 opt-in live tests skipped at 86% coverage; full Ruff and 145-file Mypy passed. No legacy writer, scientific endpoint, dependency, public export, or submission authority changed._
  - [x] 262.7 Export validated Open Science research objects
    ...
    - _Outcome: added a side-by-side, provider-neutral Open Science exporter over a verified provenance-v2 bundle. It emits RO-Crate 1.3 with explicit Workflow RO-Crate 1.0 and Process/Workflow/Provenance Run 0.5 compatibility; W3C PROV JSON-LD; CodeMeta 3.1; CFF 1.2; CRediT; an honest non-deposit-ready DataCite 4.7 draft; optional commit-matched SWHID; SPDX 3.0.1; and unsigned, no-level SLSA v1 construction provenance. Internal-complete and sanitized review/reproduction views are always separate; a public view requires scope-matched human approval, explicitly public artifacts, compatible licenses, and a clean source scan. The existing reproducibility layout, legacy writers, scientific artifacts, dependencies, publication authority, and submission authority remain unchanged._
    - _Verify result: 8 deterministic unit tests and one opt-in real-round smoke passed. The existing `task260-autonomous-ccfb-v1/round-001` bundle hash remained `a2e54556b3f6e242deeaff3d7c87400ae23e701ef034983fb6964a3c2df4c782`; seven source artifacts remained hash-bound, six frozen negative-result/decision assertions passed in a clean isolated directory, private paths were absent from the review view, and public materialization remained blocked. `rocrate-validator` 0.11.2 passed all required checks for Workflow RO-Crate 1.0 and the three Run-Crate 0.5 profiles; Workflow RO-Crate recommended checks also passed, while the other recommended profiles retained only two documented local-file/HTTP-identifier advisories. The official CFF 1.2.0 and SPDX 3.0.1 JSON Schemas both reported zero errors. Full regression passed with 866 tests and 8 opt-in live tests skipped at 86% coverage; full Ruff and 146-file Mypy passed._
  - [x] 262.8 Migrate Competition, Campaign, and Sprint as vertical slices
    ...
    - [x] 262.8.1 Migrate the Competition lifecycle as the first vertical slice
      - Freeze a versioned Competition corpus for complete, negative-result, access-blocked, failed-after-plan, blocked-then-resumed, and unchanged-terminal-idempotent behavior.
      - Add `legacy`, `shadow`, and `vnext` lifecycle-authority modes behind `AUTORESEARCH_COMPETITION_MIGRATION_MODE`; keep legacy mode behavior-identical and retain the existing manifest, artifacts, reader, and compatibility writer.
      - In shadow/vNext modes, project each distinct legacy observation into a sealed standard-event journal and acyclic Control Graph. Represent access recovery as a child journal anchored to the prior blocked seal, and refuse to append a second invocation for an unchanged terminal fingerprint.
      - Compare event semantics, terminal state, scientific endpoint, evidence gate, artifact hashes, redacted failure semantics, and access/human intervention counts. Validate journal, projection, report, and formal-run evidence before allowing cutover.
      - Require two distinct complete shadow-authority formal runs before `vnext` can start scientific work; then prove one vNext-authority vertical and a reversible read through the retained legacy path.
      - Preserve scientific semantics: an adapter exception remains a failed execution with its last valid persisted stage, access remains blocked/resumable, and a failed feasibility decision remains a negative result. Do not claim the generated fixture is official MDBench Gate A.
      - _Outcome: Competition now has a strangler lifecycle adapter over the existing scientific engine. Default legacy mode creates no migration files. Shadow mode emits immutable event journals, control projections, parity reports, promotion evidence, and terminal-idempotency observations. vNext authority is fail-closed until two independently identified formal runs pass, and the old files remain available for rollback and one compatibility window._
      - _Verify result: the frozen six-case corpus and seven deterministic migration tests passed; all 61 Competition unit tests passed. An opt-in local vertical under `runs/manual-live/task262-competition-migration-v1/` completed two distinct formal shadow runs, one vNext-authority cutover, and a legacy rollback with equal lifecycle result, equal projection, unchanged sealed journal, and preserved compatibility files. Full regression, Ruff, Mypy, and diff checks are recorded in `Agent.md`._
    - [x] 262.8.2 Migrate the Campaign lifecycle as the second vertical slice
      - Freeze Campaign-specific complete, negative-result, retained-blocked, failed, failed-then-resumed, and unchanged-terminal-idempotent semantics without borrowing Competition evidence or changing the scientific state machine.
      - Add `legacy`, `shadow`, and `vnext` authority modes behind `AUTORESEARCH_CAMPAIGN_MIGRATION_MODE`; keep the existing Campaign executor, round manifests, reports, artifacts, reader, and compatibility writer authoritative in default legacy mode.
      - In shadow/vNext modes, project every persisted stage and finalized round into a sealed standard-event journal plus an acyclic Control Graph. Preserve the last valid legacy stage on failure, redact exception text to type and message digest, and represent recovery as a child journal anchored to the failed terminal seal.
      - Compare event history, terminal state, scientific endpoint, aggregate and per-round contribution gates, artifact inventory/hash, failure semantics, and intervention counts. Revalidate reports against journal bytes, lineage, seal, projection, source fingerprint, Control Graph, and formal-report hashes.
      - Require two distinct, complete, two-round `CONTRIBUTION_READY` Campaigns with different formal IDs before vNext may begin work; then prove one projection-authority Campaign and a reversible legacy read with all compatibility files retained.
      - Keep characterization boundaries explicit: the generated complete fixture verifies migration behavior only, and the retained `BLOCKED` case characterizes a hash-valid legacy reader state that the current Campaign executor does not itself emit.
      - _Outcome: Campaign now has a default-off strangler lifecycle adapter over its unchanged scientific engine. Shadow mode records immutable stage/round events, Control Graph projections, seven parity dimensions, promotion evidence, recovery forks, and terminal-idempotency observations. vNext authority fails closed before execution unless two independently identified formal Campaigns remain fully equivalent; rollback keeps the old files and readers intact._
      - _Verify result: seven deterministic migration tests and all 35 Campaign unit tests passed. An opt-in local vertical under `runs/manual-live/task262-campaign-migration-v1/` completed two distinct formal shadow Campaigns, one vNext-authority Campaign, and a legacy rollback with equal lifecycle result, equal projection, unchanged sealed journal, and preserved compatibility files. Full regression passed with 898 tests and 10 opt-in tests skipped; full Ruff and Mypy passed._
    - [x] 262.8.3 Migrate the Sprint lifecycle as the third vertical slice
      - Freeze Sprint's own complete, negative-result, blocked, failed, resumed, and terminal-idempotent semantics before adding an adapter; do not infer them from Competition or Campaign.
      - Shadow the full topic-selection, experiment, inference, manuscript, review, and terminal lifecycle into standard events and a Control Graph while preserving the legacy sprint manifest, artifacts, readers, and scientific hashes.
      - Require service-specific parity, two distinct complete formal runs, one reversible vNext-authority vertical, and a rollback rehearsal before task `262.8` or milestone M1 can close.
      - Keep scientific and execution failure semantics separate: a completed Sprint whose frozen task-level interval misses the gate remains a negative result; a caught runtime problem remains resumable `BLOCKED`; an integrity exception that escapes the legacy preflight becomes digest-only `FAILED` without rewriting the last persisted legacy state.
      - Add `legacy`, `shadow`, and `vnext` authority modes behind `AUTORESEARCH_SPRINT_MIGRATION_MODE`; keep default legacy behavior and compatibility files unchanged, and reject vNext before independently revalidating two formal shadow observations.
      - _Outcome: Sprint is now the third service behind the default-off strangler lifecycle boundary. Shadow/vNext modes project every persisted autonomy event plus the terminal observation into a sealed standard-event journal and acyclic Control Graph, compare seven service-specific parity dimensions, fork recovery from the prior blocked/failed seal, and refuse a second invocation for an unchanged logical terminal state. Promotion accepts either a positive task-level endpoint or a fully verified scientific negative result, but still requires complete paper, autonomy, artifact, no-fallback, and no-post-start-manual-decision gates. Legacy manifests, ledgers, artifacts, reader, writer, dependencies, and submission authority remain intact._
      - _Verify result: the frozen six-case corpus and seven deterministic Sprint migration tests passed; all 42 Campaign/Sprint unit tests passed. An opt-in adoption smoke read two existing completed real negative-result Sprints as distinct formal shadow evidence without rerunning models or experiments, enabled vNext authority, projected one existing blocked Sprint, and rolled it back with equal result/projection, unchanged sealed journal, preserved compatibility files, and no private paths in migration JSON. Full regression passed with 905 tests and 11 opt-in tests skipped at 87% coverage; `ruff check src tests` and 149-file Mypy passed._
  - [x] 262.9 Add unified evaluation, observability, and Agentic security gates
    ...
    - _Outcome: added a strict, provider-neutral evaluation plane whose content-addressed task, trial, trajectory, outcome, rubric, grader, uncertainty, cost, failure, regression, fault, promotion, and rollback records keep system quality separate from scientific validity. Verified scientific negative results may complete a research trial, while unknowns, duplicate episode evidence, grader bias, evidence mismatch, holdout leakage, cost uncertainty, missing fault/regression coverage, and active candidates without a rollback target fail closed. Five local regression dimensions and ten Agentic fault cases are deterministic and independently recomputed. The local-only OTLP 1.11.0 exporter pins OpenTelemetry core semconv 1.43.0 and GenAI commit `d74a9bbc419c67dd78ea4fcc26280381ef0bb9db`, excludes raw sensitive content, hashes sensitive metadata, and permits a separate raw artifact only with an unexpired scope-bound grant. Legacy writers, dependencies, external benchmark suites, publication authority, and submission authority remain unchanged._
    - _Verify result: 21 evaluation and 8 OTel unit tests passed; the unified opt-in smoke skipped by default. An explicit adoption smoke read two existing completed real negative-result Sprints without rerunning research, passed five regression dimensions and all ten fault cases, promoted two independent verified-negative trials, emitted no raw sensitive artifact, and produced evaluation/fault/regression/OTLP hashes `b5e21a0a93e1b3caa96f4a5f5bf7ec637a09bf97305d39e9d26164324ea6d1ee`, `53f182bb856d702b5ee1bd90ec5384369ee43e6dc0910f2e15419cd972560f73`, `c2a466d01aa703d5c62a8eb47131aec0dbcc95bd424033507f67b540f14ba33c`, and `86236e468ad1a3dce58acbb02ae8054a857aee45b53f8d5becec43bb2c171e85`. Full regression passed with 934 tests and 12 opt-in tests skipped at 87% coverage; `ruff check src tests` and 151-file Mypy passed._
  - [x] 262.10 Upgrade runtime dependencies and release the vNext compatibility boundary
...
- [ ] 263. Recover publication-grade scientific search through replication-first portfolios
  ...
  - [x] 263.2 Add Research Question Certificate, opportunity, and portfolio contracts
    ...
    - _Outcome: added 16 strict, provider-neutral, content-addressed contract types for verified sources, one-claim Research Question Certificates, prospective power/disjoint panels, clean-room baseline plans/evidence, two-stage conjunctive opportunities, diverse branches, F0—F3 fidelity, bounded budgets, and independent assessments. Track selection can pass with a baseline smoke and plan, but novelty search and any 8—16-branch portfolio require independent within-tolerance baseline reproduction. Three mechanism families, one null/rule arm, unique branch evidence/deltas, non-increasing survivors, exploration quota, full branch retention, one confirmatory claim, result blindness, sealed-evidence denial, and human-gated external actions are mandatory._
    - _Verify result: 16 focused unit/property tests passed; all 54 research tests passed before the final hardening additions; deterministic export produced 16 JSON Schemas with bundle hash `47cf6a3f5c0a2cd52dfaf5f6427dfbf71efde272671de74464a6aa0e84797629`; the full repository passed with 999 tests, 17 opt-in live tests skipped, and 87% coverage; repository-wide Ruff and Mypy across 159 source files passed; `git diff --check` passed apart from an unrelated concurrent `.gitignore` line-ending warning. No live smoke applies because this slice defines local contracts and executes no external data/model surface; Task `263.3` owns the first live opportunity audit._
  - [x] 263.3 Run a live opportunity tournament across at least three research tracks
    ...
    - _Outcome: added eight strict, content-addressed live-tournament contracts for bounded resource probes, non-shell baseline execution smokes, prospective sensitivity, feasibility audits, conjunctive track assessments, deterministic no-weight ranking, reports, and artifact manifests. A real three-track tournament admitted only `track.search-policy-causality` to Task `263.4` baseline reproduction. The neural-operator track was blocked by baseline/compute feasibility and the sequential-falsification track by baseline/software-license feasibility. All tracks retained their evidence; no winner was hard-coded, all tracks could fail, and novelty search, confirmatory reveal, public release, and external submission remained false._
    - _Verify result: 12 focused tests passed; the opt-in live smoke passed once in 239.04 seconds with 11/11 primary literature URLs, 9/9 repository/data/license endpoints, and the immutable 210-cell Task `260` baseline assertion. The final report hash is `de4769b74098650a1ed7a7f92fdd853459f468d5a35e4b6d152f0169779bf0ff`, manifest hash `db810365f362de9fb06d541a7db1fc1634c1bed06d0f5b5b446e8b01a76ca932`, and eight-schema bundle hash `5609a30f5d4c9900aa8e500bcb61f4f222e7e3de553a9c81ca996239cfffe5d0`. Three Vault-link tests passed, and a line-ending-normalized audit confirmed all three Markdown summary rows are contiguous. The full repository passed with 1011 tests, 18 opt-in tests skipped, and 87% coverage; repository-wide Ruff passed and Mypy passed across 160 source files. `git diff --check` found no task-owned whitespace error and emitted only the preserved concurrent `.gitignore` line-ending warning. Failed diagnostics and their verified repairs are recorded in `P-20260729-054` through `P-20260729-060`._
  - [ ] 263.4 Reproduce the strongest baseline and preregister the search-policy causal study
    - [x] 263.4.0 Audit task, evaluator, data, license, and endpoint-specific power feasibility before reproduction
      - Re-audit every selected development and confirmatory task from official metadata without reading gold programs, gold results, or evaluation results; distinguish structured deterministic evaluation from image/model-judged evaluation.
      - Replace generic normal sensitivity with prospective two-sided exact McNemar power for the frozen paired binary endpoint; treat tasks as independent units and seeds only as within-task repeats.
      - Fail closed when task artifacts, evaluator source, anonymous data access, license, objective scoring, or adequate power cannot be established; emit a reproduction diagnosis and keep novelty search false.
      - _Verify: deterministic contract/hash/tamper/leakage tests, one opt-in live official-metadata/repository/data probe, Vault projection, Agent/Problem updates, and one focused commit._
      - _Outcome: implemented six strict, content-addressed study contracts for per-task output/evaluator/data/license audits, exact paired-binary power, complete-or-blocked baseline binding, conjunctive feasibility, reproduction-gated four-arm/five-ablation preregistration, and artifact manifests/schemas. Official live metadata fixed 102 ScienceAgentBench rows and CSV SHA-256 `7f490f17f721a9c7e9415d3608a1a37d1a5315a26862cf556e3096ac4062face`; the selected confirmation panel has 9 image/model-judged and 3 structured outputs, task-specific evaluators/full data are not anonymously auditable, and the baseline was correctly not attempted. Exact McNemar power at `n=12` is `0.054402`—`0.095619`; the frozen sensitivity set requires up to 60 independent tasks. Status is `blocked_reproduction_diagnosis`; novelty, confirmation reveal, release, and submission remain false._
      - _Verify result: 10 focused tests and 3 Vault-link tests passed; six deterministic schemas have bundle hash `4541ecc3deb3d094bb826be532354998593539132b32921e5712cdb42560ba2c`; the final opt-in official-source smoke passed in 39.02 seconds with report hash `7c4d06eb82eabb250cf1b509242480bf27f079f65eaec6fbe564593c54b4aa3c` and manifest hash `1d18b358d9b537ad083095d9897b542d6a1a8870b3b7393e6d017a41a1582a43`. The full repository passed with 1021 tests, 19 opt-in skips, and 87% coverage; repository-wide Ruff and Mypy across 161 source files passed; task-owned `git diff --check` passed._
    - [x] 263.4.1 Build an adequately powered, fully open, objective task panel
      - Inventory only tasks with anonymously downloadable data, pinned evaluator source, clear task/data/code licenses, structured deterministic outcomes, bounded compute, and no model judge in the primary endpoint.
      - Provide at least the maximum independent-task count required by the frozen exact-power sensitivity set (currently 60 for SESOI `0.25`, alpha `0.05`, target power `0.80`), or prospectively justify a different exact design before any outcomes are read.
      - Use at least two benchmark/task families, block randomization by benchmark and domain, keep development and confirmation disjoint, and retain seeds as within-task repeats.
      - Stop and retire or redesign the claim if no lawful, executable, adequately powered panel exists; do not manufacture units from seeds, subtasks, or repeated trajectories.
      - _Verify: deterministic panel-registry and leakage tests, opt-in live data/evaluator/license/compute smoke for every family, exact-power audit, full quality gates, Agent/Problem updates, and one focused commit._
      - _Outcome: implemented a frozen, outcome-blind OpenML source registry, deterministic balanced-accuracy/R² evaluator source, four content-addressed panel/probe/report/manifest contracts, and a 67-task panel with 7 disjoint development tasks plus exactly 60 confirmatory tasks (41 CC18 classification and 19 CTR23 regression). Source groups are unique; ambiguous `Public`, noncommercial, version-unspecified `CC BY`, and correlated mfeat/wine variants are excluded with reasons. Every task binds official metadata, data MD5, fixed split, source/task/code license, objective structured output, and CPU compute limits. The claim is explicitly limited to bounded tabular-ML search policy, not general autonomous science. Status is `ready_for_clean_baseline`; baseline reproduction, success thresholds, novelty, confirmatory execution, release, and submission remain false._
      - _Verify result: 7 focused deterministic tests passed; the final opt-in official-source smoke passed in 49.60 seconds after checking all 67 selected metadata/license/hash records and downloading only one development task per family. Report hash is `ab4435f059676bcfd11387495947527455734eddf239f77b0e92a1c434e8a3ac`, registry hash `6aa348b2014905d582b979dd35183fe9fa722abcbd41a8de9f65c720bafe780e`, manifest hash `2224147ea065249e10e6ad69642f91611376b0f84ec983b99b0d122028bc4efa`, and four-schema bundle hash `20ba62ede420aba6738ffac3f61d3fa4acdac6f792075bab075a85c7c2125cc4`. Full repository verification passed with 1028 tests, 20 opt-in skips, and 87% coverage; repository-wide Ruff and Mypy across 164 source files passed._
    - [x] 263.4.2 Reproduce the strong baseline and freeze the causal preregistration
      ...
      - Freeze budget-matched one-shot, linear self-loop, portfolio, and portfolio-plus-memory arms; certificate/diversity/multi-fidelity/reviewer/memory ablations; task suite; objective evaluator; nulls; independent units; exact power; cost; randomization/blocking; stop rules; and untouched confirmatory panel before search results exist.
      - Stop or retain a reproduction diagnosis if the baseline cannot be reproduced; do not build novelty on an unreliable baseline.
      ...
      - _Outcome: added strict contracts for a content-addressed clean FLAML `2.6.0` baseline, two-environment task replay, result-free paired-baseline thresholds, a four-arm/five-ablation causal study, matched F0—F3 budget, blocked randomization, sealed permissions, and a fail-closed manifest. The official live run verified 14 pinned wheel hashes, created two separate virtual environments, and reproduced all 7 development tasks in independent A/B processes with exact raw-prediction and objective-score agreement at 12 trials per run. Sixty confirmatory thresholds, 804 blocked assignments, exact McNemar/Holm analysis, and stop/failure rules were frozen while `result_record_count=0`; no confirmatory payload, public OpenML run, novelty result, release, or submission was accessed or authorized. Status is `ready_for_development_search`, not a publishable result._
      - _Verify result: 8 focused deterministic tests passed; the final opt-in official-source and clean-install smoke passed in 150.93 seconds. Baseline report hash is `e8f828c97561e789f523328aa25b82d512a159ab1e6f447f6163a770df4598e5`, preregistration hash `100f8a0054fb1fc69ef77cbdeab5521361ba5b1a514082bac9e78493fcf0e707`, manifest hash `df0324759c6099bdb1cf5764cdc4a3e5db838ae9328db0b8b427de562dc8055a`, dependency-lock hash `e03b61f59bbfeba0b6cab33d9c56611158b91913a754b61f667ecca2e77f8a51`, runner-source hash `1d7cb87d0c70887b122b5fb6bd83952562cb18d6c30c1b425d723f719116174a`, randomization hash `0e078296a7ca7b3d115f15bae8e9c3ef3d0e281a22dec9b48d2dd8e58d0ac588`, and eight-schema bundle hash `126bd2d2a840fdf2b6a6d63a487d5b6bb79d3fdc65a88d398ce7a914925c9dcb`. Full repository verification passed with 1036 tests, 21 opt-in live tests skipped, and 87% coverage; repository-wide Ruff passed and Mypy passed across 165 source files; `git diff --check` found no task-owned whitespace error._
  - [x] 263.5 Execute the budget-matched multi-branch development search
    ...
    - _Outcome: implemented 12 declarative candidates spanning nine mechanism families, including a null prior and an intentional invalid-schema control; one result-blind local Qwen call ordered that fixed catalogue, after which all 189 task-seed-policy assignments used zero model calls. The complete F0—F3 matrix retained 9,072 candidate-stage rows, 315 unique objective evaluations, 1,386 logical cache reuses, every cost/intervention/lineage record, four primary arms, and five one-component ablations. A complete v1 diagnostic matrix correctly stopped with no survivor after exposing a numeric-only evaluator defect on a mixed-type OpenML task. The v2 repair lineage binds the v1 freeze/report/failure hashes, reuses the exact prospective candidate order, and changes only frozen mixed-type imputation/one-hot compatibility; candidates, policies, budgets, thresholds, randomization, survival rules, and the confirmation seal remain unchanged. In v2, `portfolio_memory` succeeded on 6/7 development tasks versus 5/7 for `linear_self_loop`; the task-level risk difference is `+0.142857`, paired bootstrap interval `[0.000000, 0.428571]`, and exact McNemar `p=1.0`. F1→F3 and F2→F3 task-level Spearman correlations are both `0.964286`. All five frozen survival checks pass, so the sole preregistered policy is `ready_for_confirmation`; this is a development-screening decision, not a significant effect or publishable claim. All 21 retained failures belong only to the reviewer-ablation intentional negative control. Sixty confirmatory payloads remain untouched, and release/submission authorization remains false._
    - _Verify result: freeze hash `1120bc27839eafefcf20e042e7b043e344c9d59cc3b2daa657a102c5ff264332`, report hash `b767a0963d0c4f60a92cbc7c35b835918122028f90bff5bb6b73e43ccecd1123`, and manifest hash `e423e7cc3f82d083c8a0776f572a550da0cad06fd7b70b79b3d2f213fe71eb49` verified recursively. Exact `run`/`verify` repetition preserved both report and manifest hashes. The opt-in real matrix smoke passed in 7.08 seconds against the completed artifact; a clean-interpreter mixed-type probe produced 20/20 finite predictions with an unseen category. Thirty focused unit tests passed. Full repository verification passed with 1,046 tests, 22 opt-in skips, 86% coverage, repository-wide Ruff, and Mypy across 166 source files; `poetry check` exited zero with pre-existing metadata deprecation warnings._
  - [ ] 263.6 Execute untouched confirmatory evaluation and independent adjudication
    ...
    - _First one-use outcome: `invalid_confirmation`, retained without reinterpretation. The primary and clean-room executions each completed 1,620 policy assignments plus 180 null controls and produced the same scientific projection. The frozen primary comparison was unfavorable to `portfolio_memory`: 26/60 task successes versus 28/60 for `linear_self_loop`, risk difference `-0.033333`, conservative exact 95% interval `[-0.153229, 0.093699]`, and exact McNemar `p=0.625`. The endpoint cannot be called a credible negative because 69/180 null-control rows failed integrity on exactly 23 classification tasks × three seeds: CSV inference made training labels numeric while the sealed F3 labels remained strings, so the frozen runner raised `Mix of label input types (string and number)`. The panel is consumed; the frozen v1 result, source, route, and release gates remain unchanged._
    - [x] 263.6.0 Preserve and diagnose the first invalid confirmatory endpoint
      - Retain the complete invalid report, raw matrix, null controls, technical interruption ledger, exact clean-room replay, and all content hashes without editing the frozen v1 scientific source.
      - Distinguish the observed failed main-effect gates from the validity failure; do not relabel the endpoint as a credible negative or use the consumed panel as untouched evidence again.
      - Make the opt-in live smoke accept positive, credible-negative, or invalid status according to the validity conjunction rather than preordaining a favorable terminal class.
      - _Verify: recursive public-loader reconstruction, exact primary/replay scientific projection, failure-pattern/root-cause audit, validity-aware live smoke, frozen-source hash check, documentation/Vault/Problem/Agent updates, and one focused commit._
      - _Verify result: recursive loading bound report `664993d04132dbfcff7aacb7431e499103c0698c2282c5325a6a42000401513a`, manifest `c9c7e2993d3be15894579ee50867a7e1511184027d7cd2fcde427dabc2924567`, freeze `7069ae95433cf7f83c86d35993dd3bd88020e919102d01594574c1860b3c8031`, and clean-room projection `17299042a7f3b851b7e16fdea183e6cd6c9622833bfb678277d001b96d570789`. The repaired live-smoke assertion passed against the immutable completed artifact in 178.47 seconds._
    - [x] 263.6.1 Build a v2 evaluator-compatibility certificate before any new confirmation
      - Canonicalize classification targets across ARFF/JSON/CSV boundaries, distinguish candidate failure from evaluator failure, and prove dummy/prior, every allowed learner, F3 label isolation, and prediction replay over numeric, string, sparse, dense, quoted, mixed-type, and unseen-category fixtures in both pinned interpreters.
      - Fix the already-materialized task-bundle resume branch in the next orchestrator version; never modify or refreeze the v1 endpoint.
      - _Verify: deterministic fixture/property tests, all-family clean-interpreter probes, null-control zero-integrity certificate, exact replay, source/package hashes, and no confirmation-result access._
      - _Outcome: added a standalone, network-free v2 confirmation runner that reads classification targets with lexical string semantics, scores F3 labels in the same encoded vocabulary, rejects any label binding at F1/F2, and emits structured `input`, `candidate`, or `evaluator` failure domains. Added a result-blind certificate orchestrator that reads only the v1 freeze and frozen assets, never the consumed task bundles or results; materializes and hash-verifies four deterministic dense/sparse ARFF fixtures spanning numeric-looking and string class labels, quoted commas, mixed features, unseen test categories, and regression; reconstructs feature columns on the already-materialized resume branch; and recursively inventories every config, input, log, result, schema, source asset, report, and Markdown artifact. The two pinned interpreters completed 144 valid F3 probes covering dummy/prior plus every valid learner/configuration, four expected invalid-probe controls, and four physical F2 label-isolation probes. All 15 conjunctive checks pass; null-prior, unexpected-candidate, evaluator, and input integrity-failure counts are all zero; within-interpreter prediction replay and cross-interpreter scientific projections are exact. The frozen v1 endpoint and all protected v1 source hashes remain unchanged. This certifies the measurement boundary only; it does not repair the consumed v1 endpoint or establish a scientific effect._
      - _Verify result: report `e3709c8b834bfcc52ed7fb74389278e6c5a3e36d4bf13d32ddad7118f4aa797b`, manifest `4e3251eb2453fffaa37a4f6849251396e3f1fc88f882739faa07a5e8d4dda73c`, v2 confirmation runner `7bc71786216ba48addc3ab4212c2b18b9ffd1ed0da3a60cc18b942d23afc77fd`, orchestrator `97c2152427c7a075731fb83fe421351ae7f9728d60d691f3e867a54e4b1fd1bc`, and four-schema bundle `6b9a4353a819ba4f7abf8354b8c43b07b67b568327f9938240436f1a0e8b1d3` verify across 1,242 recursively hashed artifacts. Eight focused deterministic tests passed; the opt-in two-interpreter live certificate passed in 222.72 seconds. Canonical full regression passed with 1,066 tests, 24 opt-in skips, and 84% coverage in 173.72 seconds; repository-wide Ruff, Mypy across 168 source files, Poetry validation, and diff checks passed._
    - [x] 263.6.2 Run a technical-only repair replay on the consumed panel
      - Re-execute the exact frozen claim with the certified v2 evaluator only to diagnose the v1 measurement failure; label every output exploratory/technical and forbid it from satisfying independent-confirmation or publication gates.
      - Require a frozen stop/advance certificate. If the corrected main effect is not directionally positive and practically plausible, stop the `portfolio_memory` publication claim rather than shopping for another panel.
      - _Verify: v1-to-v2 repair lineage, complete replay, failure/cost comparison, no route change, and an explicit consumed-panel marker._
      - _Outcome: froze the exact consumed-panel lineage and a pre-result conjunction that could at most permit new-mechanism review. Primary and replay each completed 1,620 assignments plus 180 null controls, but the frozen orchestrator correctly refused to emit a formal technical report because eight assignment scientific projections differed. All differences trace to one task/seed workload boundary where `xgb-deep` F1 succeeded in primary and reached the 60-second deadline in replay; all 180 null projections match. A separate immutable `invalid_technical_replay` incident records 31 incomplete label-access attestations without mislabeling them as leakage. Its non-inferential diagnostic is unfavorable: `portfolio_memory` 40/60 versus `linear_self_loop` 43/60, risk difference `-0.05`, conservative exact 95% interval `[-0.196505, 0.105569]`, exact McNemar `p=0.453125`, 2 favorable versus 5 unfavorable tasks, both family effects negative, 30 unexpected candidate failures, and 15 infrastructure timeouts. The frozen decision is `stop_portfolio_memory_claim`; no result-contingent route change, new confirmation, publication, release, or submission is authorized._
      - _Verify result: repair freeze `6b7f124fab513e8032ff777b2a92926cf5e57836d409ad133700c49946cea22b`, incident `f756ab01b1e7291875470e75d63e5fe668bf199a50659c041799e038578f9dd0`, diagnostic analysis `f599ed894e484dae483c25e27364ebea5ceec27f45c925bfc625e16fed0d08b3`, and 36,521-artifact incident manifest `79bfb70fa5ded53686ada5deadb1e735450ad442a441867b93eef615a9c30fe6` reconstruct exactly. Five incident and six technical-replay unit tests passed; the opt-in retained-artifact smoke passed in 74.29 seconds. Canonical full regression passed with 1,077 tests, 26 opt-in live skips, and 82% coverage; repository-wide Ruff passed, Mypy passed across 170 source files, Poetry validation exited zero with pre-existing metadata deprecation warnings, and diff/link/hash audits passed. The frozen controller and replay-orchestrator hashes remain `f7a561542eb30b18fb4369fdf1d318de0d22ce97b3c2465276ff121465299ced` and `924c3e0a7cab8c588870b542956881524e386a85cea3c1baa22049fe00185e65`._
    - [ ] 263.6.3 Register a new claim and untouched panel only after the recovery gate
      - If and only if 263.6.2 supplies a result-blind, mechanism-based advance rationale, return through opportunity/development selection and freeze a new independent panel; otherwise close this claim and select a different objective research track.
      - Never present the v1 panel, the v2 technical replay, or a newly selected favorable subset as fresh confirmation.
      - _Verify: new question/opportunity certificate, disjoint source groups, prospective power, zero-result freeze, evaluator certificate binding, and explicit human review before another one-use reveal._
      - _Gate result: not entered. Task 263.6.2 failed the pre-frozen advance conjunction and closed the current claim. A new panel for `portfolio_memory` is forbidden; future work must use Task 263.6.4 to select a genuinely different question/mechanism before any new confirmation design._
    - [x] 263.6.4 Re-enter an objective opportunity tournament with workload qualification
      - Compare at least three fresh, mechanism-based, objectively scorable tracks without using the consumed v1/v2 panel or its outcomes for candidate ranking: structured world-model/evidence-graph coherence, Socratic causal/constraint/counterexample/falsification critics, and external data/lab/environment feedback with explicit human scientific responsibility.
      - Add a result-blind `WorkloadQualificationCertificate` between evaluator calibration and scientific freeze. On development-only data, repeat the slowest representative task/candidate strata across both interpreters and planned concurrency; separate algorithmic compute budget from orchestration deadline; freeze timeout slack, telemetry, retry, and exact-versus-tolerant replay semantics; reject tracks whose scientific trajectory changes at the runtime boundary.
      - For every track, require verified nearest work, a clean strong baseline, objective evaluator, independent-unit and prospective-power plan, open data/code/license evidence, a bounded compute envelope, predeclared falsifiers and ablations, full failure retention, and a result-blind publication endpoint. All tracks may fail.
      - Do not add a new confirmatory panel until one new Research Question Certificate has new development evidence and passes the non-compensating opportunity, runtime, evaluator, power, and human-review gates.
      - _Verify: refreshed primary-source audit, deterministic contracts/tamper tests, opt-in live development-only workload probes, exact repeated projections or a predeclared tolerance certificate, Vault/Problem/Agent updates, and one focused commit._
      - _Outcome: implemented a result-blind three-track tournament and a standalone standard-library workload probe. Every track calibrates and then qualifies across two distinct clean interpreter installations, concurrency 1/2, and three qualification repeats; algorithmic work units/CPU are separate from the post-calibration orchestration deadline, retry is zero, scientific projections are exact, telemetry is tolerant, and every nested artifact is content-addressed. The live audit covered Kosmos, Graph of Trace, Code as Agent Harness, AHOIS, POPPER, DiscoveryBench, AstaBench, SciAgentArena, Robin, Execution-Grounded Automated AI Research, and EurekAgent plus official repository/dataset/license surfaces. Structured world model fails open-system, strong-baseline, objective-evaluator, data/license, power, and compute gates. External feedback fails license, compute, and exact-power feasibility with only two accessible environments. Socratic falsification alone receives development admission because 189 provisional DiscoveryBench source-group folders make an exact paired design feasible against the 84-unit primary requirement and AstaBench/DiscoveryBench supply a clean-room legal boundary. That admission authorizes only baseline/evaluator construction: baseline implementation, evaluator implementation, scientific independence audit, Research Question Certificate, confirmatory panel, novelty search, publication, release, and submission remain false._
      - _Verify result: report `13e31dbe29f2d34ec3924459207610f04618271ed21ce551e13a3d0b7716e72c`, manifest `8461c05491ca487443b0a0ab5250a048ac7721d53e61af2199654ce02824e933`, and frozen workload runner `c109d368cd64cd5356cc95304948ed9d6594a823b0bddf00fa4faaa797e6bcca`. DiscoveryBench live inventory returned 987 entries, 189 provisional depth-four source-group folders, and ODC-By metadata; AstaBench license evidence resolved to Apache-2.0. All three 24-observation workload matrices completed without timeout and with exact projections. Six focused unit tests and the opt-in live test passed. Canonical full regression passed with 1,083 tests, 27 opt-in skips, and 82% coverage; repository-wide Ruff passed, Mypy passed across 171 source files, Poetry validation exited zero with pre-existing metadata deprecation warnings, Vault links passed, and diff/frozen-source audits passed._
    - [x] 263.6.5 Build the licensed objective Socratic development vertical
      - Before reading any new scientific outcome, audit all 189 provisional DiscoveryBench folders for source provenance, derivation, duplicate/raw/processed relationships, real-versus-synthetic family, answer-key lineage, and license scope. Require at least 30 disjoint development groups plus 84 untouched potential reserve groups after clustering; otherwise stop this track.
      - Build a clean-room deterministic binary valid-versus-fault-injected evaluator whose frozen fault classes include causal inversion, constraint violation, counterexample omission, and unfalsifiable claims. Do not use free-form semantic matching, an LLM judge, or post-result human labels as the primary gate.
      - Reconstruct an Asta-style same-model/same-tool strong baseline and compare no-critic, rule/null, four single-component critics, and the full critic under equal token/call/CPU/wall-clock/failure budgets. Reuse only license-clear AstaBench interfaces and ODC-By data; do not copy unlicensed POPPER/SciAgentArena code or gated data.
      - Run only on development groups, retain all trajectories/failures/timeouts/cost/human inputs, use source group as the statistical unit, and treat seeds/repeats as technical observations. Keep the provider configurable by `base_url`, `api_key`, and `model_name`.
      - Issue a new Research Question Certificate only after live evaluator/baseline reproduction, workload qualification, exact replay, prospective power, null/Type-I control, nearest-work delta, and explicit human novelty review pass. Do not create or read a confirmatory panel in this task.
      - _Verify: deterministic evaluator/fault-generator/tamper tests, license and provenance inventory, provider-neutral config tests, opt-in live development-only baseline/evaluator smoke, exact cross-interpreter projections, Vault/Problem/Agent updates, and one focused commit._
      - _Outcome: completed at the first prospective stop gate. A frozen, result-blind standard-library runner and strict inventory contracts audited the exact `allenai/discoverybench@e54ec033049d3a0fd95d3c746919cc8c01c25781` tree, answer-key dataset names, provenance/derivation groups, duplicate object signatures, and license scope without retaining gold-hypothesis text or reading development outcomes. The 189 provisional folders comprise 14 real and 175 synthetic folders, but conservative clustering yields only 8 real source groups plus 99 synthetic semantic-tree groups, 107 total; train/test group counts are 67/41 with one cross-split group. After 30 development groups, the maximum reserve is 41. Even an intentionally optimistic upper bound is only 103 development and 81 reserve, below the frozen 84 reserve requirement. The decision is `stopped-at-inventory` and returns to an objective-data opportunity tournament. Fault generator, evaluator, baseline, provider configuration, RQ, panel, public release, and submission remain false._
      - _Verify result: formal report `a01303685e1aa4ee2d6ef19f75b5ca01cf3694bc58075008d78840d9bab1d75e`, manifest contract `8253096b08a8c44c6ec99ea9286872efe76b23f376ce63097ebebb561b6e7ed2`, projection `8ec78def64fcdc4934d69cc8371d9c05a95c21299cde19ad8e00650bc46474f3`, replay certificate `02af0e8a089104da4f77e65ad9a90055aacc46e6e47d4736ebc08fa8fb2edc9b`, replay input `01cb2537f55a28a39f6a7174a6b772391ab5f9682a5b1d39bf9698ace54f4545`, and frozen runner `efe05a01434bffae461a2e2facf8afd25b085052c184ba291ebaf13e54131238`. All 85 test folders have answer-key key lineage; real/synthetic key files contain 239/200 rows and decode strictly as UTF-8-SIG/Windows-1252. Application logic does not inspect, project, or persist `gold_hypo` field values. The opt-in official-source smoke passed in 35.76 seconds with exact zero-retry projection across two clean interpreters and no development-outcome access. Eight focused unit tests, repository gates, Vault/Problem/Agent updates, and frozen-source checks passed._
    - [x] 263.6.6 Re-enter a replacement objective-data opportunity tournament
      - Compare at least four non-hardcoded candidate panels before any model call: AutoSDT-5K repository-group scientific coding, ScienceAgentBench publication-group scientific programming, CORE-Bench paper-group computational reproducibility, and QRData source-sheet statistical/causal reasoning. Preserve construct differences and allow all candidates to fail.
      - Freeze exact revisions and build a result-blind task-to-repository/publication/shared-data/generator-template lineage inventory. Exclude seeds, attempts, difficulty variants, tasks from the same underlying source, and agent votes as independent units.
      - Bind per-task or per-source license evidence and separately record local execution, software reuse, derivative, and content-redistribution scope. Do not treat a public repository or an `academic use assumed` statement as permission; fail closed on missing or incompatible licenses.
      - Require a deterministic executable primary endpoint and a coherent construct. LLM judges, free-form semantic similarity, and post-result human labels may be secondary only. Audit strong-baseline availability and bounded local download/dependency/CPU/GPU/wall-clock feasibility without running candidate models or reading reserve outcomes.
      - Require at least 30 disjoint development source groups plus 84 completely sealed reserve groups for one candidate before authorizing evaluator or critic construction. Do not combine heterogeneous benchmarks after seeing availability or results; any multi-benchmark design must be frozen prospectively with source-overlap and construct-equivalence evidence.
      - Keep Research Question issuance, provider credentials, confirmation-panel creation, release, publication, and submission false until a candidate passes license, independence, objective-label, baseline, compute, nearest-work, and explicit human-novelty gates.
      - _Verify: frozen primary-source/resource revisions, deterministic inventory and license parsers, pseudoreplication/tamper/all-candidate-failure tests, opt-in official-source live smoke, exact cross-interpreter projection, Vault/Problem/Agent updates, and one focused commit._
      - _Outcome: completed as a prospective `all-candidates-rejected` decision before any candidate model call. Exact official releases were frozen for AutoSDT-5K, ScienceAgentBench, CORE-Bench, and QRData; prompt, answer, reference-program, model/judge output, and reserve-result values were discarded before the replay projection. AutoSDT's frozen file contains 5,148 tasks and 1,317 repositories; excluding 315 `None` license labels leaves 1,002 capacity groups, but source revisions, exact source-license objects, packaged deterministic scorers, strong baseline, bounded compute, and reserve seal fail. ScienceAgentBench has 102 tasks, 30 direct repository groups and a 44-publication upper bound, leaving only 14 potential and zero sealed reserve groups; full evaluation includes GPT-4o visualization judging/best-of-three attempts and unresolved upstream rights. CORE-Bench's 270 difficulty tasks collapse to 90 paper groups, with 45 development and 45 encrypted sealed test papers; its deterministic endpoint and baseline pass, but reserve is below 84, capsule rights are unbound, and privileged Docker/GPU/cloud workload is not bounded local execution. QRData's 411 questions collapse to 190 shared data-file-set groups and its deterministic scorer/local workload pass, but 195 upstream sheets lack per-source license objects, the official baseline command is absent, and co-located public answers leave zero sealed reserve. No heterogeneous post-result combination, RQ, provider credential, evaluator/critic, panel, publication claim, release, or submission was authorized._
      - _Verify result: formal report contract `292899ec660d38490fd95dd40c832e304f6c816a1dd5f9f401b19f6615eea89a`, projection `265d8c1b1195f6ad488a2d2fe12dd5133afaeadfd18d109fff56edefd11c7491`, decision `ca0697f34a8b70e81beb67bc2960fa5c121615cba556c372307f9209a7bd9d36`, replay certificate `40370c725a9450ea3886ce0c72ad658100c27eea5f7a5c5e1a4eafbc08fced99`, frozen runner `a34a4ab5ec95fa5e37fd3f0b03c64830c18cb1449a9bc10788b5266f0707a396`, and manifest contract `4e4a47495d23f44c3df72cb3005cb4846d5f356f65b606a9677fd1c80013fc9a` verify. Eight deterministic tests cover four parsers, pseudoreplication, sensitive-outcome exclusion, all-fail/no-hardcoded-winner behavior, scope-specific license failure, exact runner parity, and tamper detection. The opt-in official-source smoke passed in 144.45 seconds after hashing every frozen artifact, probing 11 primary pages, and obtaining the exact projection in two distinct clean interpreters. Focused regression passed 31 tests with one opt-in skip; the full repository passed 1,099 tests with 29 opt-in skips and 82% coverage in 186.77 seconds. Repository-wide Ruff, Mypy across 173 source files, Poetry validation, Vault/link checks, and diff checks passed._
    - [ ] 263.6.7 Freeze and execute an AI-scientist benchmark-validity systematic mapping study
      - Treat the four Task `263.6.6` candidates as protocol-development pilots only. Before extracting any additional benchmark record, freeze search databases/pages, exact search strings, cutoff, deduplication, fixed-revision release unit, inclusion/exclusion rules, extraction codebook, unknown semantics, conflict handling, analysis, stopping rules, and negative publication endpoint.
      - Build a primary-source census of at least 20 independent AI-scientist/scientific-agent benchmark releases across literature discovery, scientific programming, data analysis, hypothesis validation, computational reproduction, experiment execution, and full research lifecycle. Preserve construct strata; do not average heterogeneous tasks into a mechanism score.
      - Produce one machine-readable Benchmark Admission Card per release with headline task count, independently lineaged source-group upper bound, task-to-unit compression ratio, exact revision and artifact hashes, local-execution/software-reuse/derivative/content-redistribution rights, deterministic primary endpoint, LLM/human judge role, official strong-baseline command, dependency/CPU/GPU/cloud envelope, split seal, contamination policy, and human responsibility boundary.
      - Require dual independent human coding and preregistered adjudication for ambiguous license, lineage, construct, and seal fields; report agreement and evidence coverage before synthesis. Automated extraction may propose records but cannot issue a legal, authorship, release, or submission decision.
      - Freeze descriptive primary endpoints: per-gate pass rates, complete-conjunction pass rate, task-to-independent-unit compression distribution, and missing-evidence rate. Add prospective construct/source/year sensitivity analyses and retain every excluded, unknown, conflicting, and unreachable record; do not convert this mapping into a causal critic/Agent-effect claim.
      - If fewer than 20 fixed-revision releases are recoverable, critical-field agreement/coverage misses its frozen threshold, or the protocol changes after extraction, stop at an open resource or diagnostic negative. If a fresh single-construct panel passes every admission gate, authorize only a new clean-baseline/RQ preregistration task; do not run candidate models or create a confirmation panel here.
      - Keep Task `260` Route B as a separate systems-paper candidate for independent human review. Keep Task `263.6.3`, mechanism confirmation, Task `263.7`, public release, and external submission behind their existing gates.
      - _Verify: pre-extraction protocol hash/timestamp, primary-source search log and deduplication replay, deterministic/mock parsers plus opt-in live source census, dual-code agreement artifact, exact cross-interpreter descriptive analysis, Benchmark Admission Cards/schema/manifest, Vault/Problem/Agent updates, and one focused commit._
      - [x] 263.6.7.1 Freeze the pre-extraction benchmark-validity protocol
        - Freeze the 2023-01-01 through 2026-07-31 search window, arXiv/OpenAlex/Crossref/DBLP source-specific queries, one backward plus one forward citation round, known-item recall gate, primary-source hierarchy, eligibility, deduplication, and latest-pre-cutoff fixed-revision release rule before executing any non-pilot query.
        - Treat a unique benchmark family as the independent unit and technical tasks, seeds, attempts, difficulties, votes, and related revisions as nested observations. Exclude the four Task `263.6.6` pilots from the primary cohort and require at least 20 additional non-pilot families.
        - Freeze a 42-field Benchmark Admission Card codebook, seven explicit evidence/unknown states, twelve non-compensating gates, four descriptive primary endpoints, six sensitivities, ten stop rules, and a negative open-resource endpoint. Only `verified-pass` can satisfy a gate; missing, unreachable, ambiguous, conflicting, and not-applicable evidence never becomes a pass.
        - Require two independent human reviewers plus a distinct human adjudicator before screening or critical extraction. Freeze 100-percent dual screening/coding, exact agreement `>=0.90`, Cohen kappa `>=0.80` when estimable, overall critical coverage `>=0.90`, and per-field coverage `>=0.85`; do not fake identities or agreement with an Agent.
        - _Outcome: completed as `frozen-pre-extraction` with zero search execution, zero extracted non-pilot records, zero benchmark-outcome access, and zero candidate-model calls. The result-free protocol binds 28 source/lens queries, 16 recall sentinels, PRISMA/PRISMA-S reporting, systematic-mapping taxonomy, BenchmarkCards, Asta-style Harness separation, POPPER-style stopping, FAIR4RS, PROV-O, and RO-Crate. The formal protocol hash is `ed6088c225d5c7f7710ecb69507659003b5b97e06dc7c0ee005a81ed2712e8ed`; report `0ed7f637ab10b10cc6b265c60020437255f64cc8d8a7259ad9eae9c9051a9408`; projection `e8628d484cfd3d5ead9dbb9b0e6610ca4f68adeebda4d0ef463bc3ac1d5e1881`; two-clean-interpreter replay certificate `85e8ee4da9ea685b32f1896759e5235bec3e47fa59af8b12e0790f9026d9b93a`; runner `fb7c4f4e535a7168a89c48fc77a28772afd931e0cd61d2df29a6d62a6c8dee6f`; manifest `9b99c6e4ccb43ea4982c546ebf6e18a34df63ae3f474ace3ed58ee2464a96b77`._
        - _Verify result: seven focused deterministic tests passed; the opt-in real smoke passed in two distinct clean Python installations; result-bearing replay payloads, pilot leakage, missing source/lens bindings, protocol tamper, and persisted artifact tamper were rejected. Ruff and Mypy over the research package passed. Formal output is `runs/manual-live/task263671-benchmark-validity-protocol-freeze-v1/`._
      - [x] 263.6.7.2 Build the result-blind search, deduplication, and evidence-packet harness
        - Implement deterministic source adapters, raw-response hashing, append-only PRISMA-S search logs, known-item recall, paper/family/revision deduplication, frozen inclusion/exclusion forms, and empty Benchmark Admission Card evidence packets without changing the committed protocol or opening benchmark outcomes.
        - _Verify: mocked adapters and pagination/rate-limit/retry tests, duplicate-family and pseudoreplication tests, protocol-deviation/tamper tests, opt-in API capability smoke that preserves raw responses, exact replay, Vault/Problem/Agent updates, and one focused commit._
        - _Outcome: completed as `ready-for-capability-only`. The Harness implements deterministic arXiv/OpenAlex/Crossref/DBLP metadata adapters, exact public-request hashes, content-addressed raw bodies, chained append-only page/execution logs, paper identity deduplication, explicit family/revision clustering, 16-sentinel recall evaluation, frozen screening forms, and a 42-field empty evidence-packet template. Four real one-page API probes retained four raw responses and four bibliographic records; formal search, Admission Card, benchmark-outcome, and candidate-model counts remain `0/0/false/false`. Crossref's frozen last-cursor rule is now an explicit formal blocker rather than a silent protocol mutation._
        - _Verify result: eight deterministic tests passed, including pagination, retry/rate-limit, tamper, duplicate-family, pseudoreplication, empty-packet, and frozen-runner rejection paths. The opt-in real API smoke passed for all four sources and reproduced projection `30bdad36006badccca89f335ff092e34c2c7f3a5a4586e5aba982689c7ba8b2d` in two distinct clean interpreters; report `fbb2a633bb57f0bb9f9f1471b58e8b4b8367098923f07c052d712758cbef9a10`, replay certificate `29ed35c21eeeea9abf3e6256b717d963d3b8fd797326cd56acd17069b31b77f8`, and manifest `688599b0b46c1502c79e9046f53dd96183989f6fcba8134bc8491d26eef18b3f` load exactly from `runs/manual-live/task263672-benchmark-validity-harness-v2/`._
      - [x] 263.6.7.2.1 Freeze the pre-extraction API-pagination erratum
        - Preserve Task `263.6.7.1` as immutable history, but bind an additive signed erratum to its protocol hash before any formal query: Crossref starts cursor paging with `cursor=*` and stops on a short page because current official documentation returns `next-cursor` even on the last page; OpenAlex starts with `cursor=*` and requires null `next_cursor` plus an empty result page for exhaustion. DBLP keeps the exact frozen `q/f/h/c` request and metadata-year post-filter; because the official API/search syntax documents a 1,000-hit cap but no year-field filter, a capped response is retained as partial and triggers a prospective diagnostic stop instead of inventing year-qualified queries.
        - _Verify: primary documentation snapshot/hash, zero formal query/extraction/outcome/model counts, deterministic erratum schema and deviation ledger, mocked boundary tests, bounded live capability checks, exact two-interpreter replay, immutable original protocol/source hashes, Vault/Problem/Agent updates, and one focused commit._
        - _Outcome: completed as `frozen-pre-extraction-erratum`. The original protocol/source and completed parent Harness report/projection/manifest hashes remain immutable. Four primary documentation pages are content-addressed; two protocol deviations are explicit; arXiv is unchanged; OpenAlex exhaustion is fail-closed; Crossref formal retrieval is authorized only through the bound erratum; and a capped DBLP response becomes `dblp-cap-retained-partial-stop`. Formal search, bibliographic extraction, screening, Admission Card, benchmark outcome, model call, human identity, publication, release, and submission activity remains zero/false._
        - _Verify result: seven deterministic erratum tests and the eight prior Harness tests passed; the opt-in live documentation smoke retained four real official pages and reproduced projection `b36624099cdda8030548068290596c41411b8e4bbc15611e3db519b2add79e7c` in two distinct clean interpreters. Report `3fefa90f73c5e6990f1817c0a06f33707b8a5e553f344a321cab18451f50310b`, erratum `f0ffc351a43eb8ac0176cca787ad53f9af4e343cc2554aca068a20215f81d571`, replay certificate `f2e83a372927b8dbebec5c48974c7b6a46d997205d8a67eaf2fe9de2c97d98c8`, integrated Harness source `f22c9bbc2a528d2ae9ab58a96ca4ddcdb4cc26fb0158deba458251d4e22fe227`, and manifest `a62d742e9466369eb5e573871b413e6c71a9aee3fff1a1e44d178593facc3ffd` load exactly from `runs/manual-live/task2636721-pagination-erratum-v2/`._
      - [x] 263.6.7.2.2 Freeze the result-blind human-review handoff and dual-lock ceremony
        - Bind an additive handoff protocol to the immutable Task `263.6.7.1` protocol, the Task `263.6.7.2.1` erratum report/projection/manifest, and its focused parent commit. Materialize exactly two independent reviewer role slots plus one distinct adjudicator slot, but leave all three identities and assignments empty until the project owner supplies real natural persons through private, non-repository identity evidence.
        - Separate public hash-only role receipts from private identity, qualification, conflict-disclosure, and consent records. Automation may validate schema, hashes, pairwise distinct opaque person identifiers, role separation, and owner-attestation presence; it must explicitly report that it cannot establish natural-person status, truthfulness, legal independence, or informed consent by itself.
        - Freeze reviewer-A and reviewer-B packet visibility so each can see the protocol, bibliographic/source evidence, frozen forms, and only their own draft/lock. Require content-addressed independent locks over the same candidate-set hash before either code stream is disclosed. Open the adjudicator packet only after a valid dual-lock barrier and only for conflicts; never use adjudication to repair failed agreement or coverage gates.
        - Persist role requirements, private-field inventory, packet templates, stage-transition policy, owner enrollment checklist, JSON Schemas, result-free projection, two-clean-interpreter replay, Markdown report, recursive manifest, and tamper-blocking loader. Formal search, screening, critical coding, adjudicator access, Admission Cards, benchmark outcomes, candidate-model calls, publication, release, and submission remain zero/false.
        - _Verify: deterministic parent-binding, empty-role, role-distinctness, private/public separation, cross-reviewer leakage, dual-lock ordering, early-adjudicator, forbidden-result, replay-parity, persistence, and tamper tests; opt-in local-only smoke using the real parent erratum and two clean interpreters; Vault/Problem/Agent updates; and one focused commit._
        - _Outcome: implemented as `frozen-result-blind-human-handoff`. The public package materializes exactly three unassigned slots, seven private-only enrollment fields, isolated reviewer packet templates, five stage transitions, a same-candidate-set dual-lock barrier, and conflict-only adjudicator access. Future hash-only receipts must bind the immutable protocol, handoff, role requirement, packet template, and distinct opaque person IDs. Automation can validate structure and hashes but explicitly cannot establish natural-person status, truthful disclosure, legal independence, qualification, or informed consent._
        - _Verify result: 29 protocol/Harness/erratum/handoff tests and the opt-in local-only two-clean-interpreter smoke pass. Report `c070839d39aa9b5a5b18af170e4b7c8690faf342399c1c98a2ef13ecba0f17b7`, handoff `2abc9296b2b14471ad8236e1d91b501f9c6c320950a3552ac771409b4df9fa18`, result-free projection `bf9298474bddd74dc274984c474e5d27b92f8cea578b7a2963b6f1841976c3f5`, replay certificate `17c57008cdf404c1ecbe74ab670773775d881dc6811d709dca53532ca1c1d259`, source `c643266e298d2bc7e39f643a9cd0ddb5420a7ae1892afa9b9f131220304c15f5`, runner `dd4666051fe2f9c0548712cac884994832540f56549874f5af8277be78ed1c63`, and manifest `1060176b4d23cf13ca5cbde23d8f664adfdc9334048adbd7737d2926abf6c6a1` load exactly from `runs/manual-live/task2636722-human-review-handoff-v2/`. Identity, assignment, lock, adjudicator-access, search, screening, coding, Admission-Card, outcome, and model-call counts remain zero; all formal-census/publication/release/submission permissions remain false._
      - [ ] 263.6.7.3 Execute the frozen census, dual human coding, and descriptive synthesis
        - Assign two real independent human reviewers and one distinct human adjudicator, execute all frozen searches, retain all screening/exclusion/conflict/unreachable records, construct at least 20 non-pilot fixed-revision release cards, and compute only the frozen descriptive/sensitivity endpoints.
        - _Verify: complete search/dedup replay, known-item recall, human identity/independence attestations, pre-adjudication agreement and coverage gates, adjudication ledger, exact cross-interpreter analysis, card/schema/manifest/Open Science package, Vault/Problem/Agent updates, and one focused commit or the frozen diagnostic-negative stop._
  - [ ] 263.7 Build the claim packet, paper, and Open Science research object
    - [x] 263.7.0 Audit the frozen Task 260 systems paper against the current field and independent-unit inference
      - Bind the additive audit to immutable Task `260` v2 evidence: focused parent commit, package directory and package hash, compiled PDF hash, systems-result hash, and systems-gate hash. Preserve that package byte-for-byte and keep external submission, publication, release, authorship, license, and venue decisions false or human-owned.
      - Freeze three result-blind questions before analysis: whether the Route B claim remains differentiated after 2025-2026 autonomous-science systems and benchmarks; whether `10 tasks x 3 deterministic seeds` supports independent-unit inference; and which repairs are possible from frozen evidence versus requiring new independent experiments or human judgment.
      - Cross-search at least five adversarial perspectives using primary sources: mainstream autonomous-science systems, independent benchmarks and failure audits, statistical/benchmark methodology, Open Science interoperability, and critical human-responsibility or integrity work. Content-address retrieved source evidence, distinguish peer-reviewed articles from preprints and editorials, and bind every novelty or limitation finding to a verified locator.
      - Recompute the paired effect at the independent task level, prove whether seed-level scientific outputs are duplicates, report the preregistered 20,000-resample task bootstrap, exact sign test, task-family sensitivity, and the contrast with the frozen 30-pair interval. Do not silently substitute a new primary endpoint into the immutable Task `260` package.
      - Perform a retrieval-grounded pre-submission audit across logic, novelty, empirical validity, external validity, reproducibility/Open Science, and language/LaTeX hygiene. Classify every repair as existing-evidence text revision, existing-evidence reanalysis, new-independent-evidence work, or human-only decision; unresolved critical or major defects must prohibit a publishable/ready verdict.
      - Persist the frozen brief, verified source registry and raw snapshots, independent-unit audit, quoted paper findings, repair plan, machine-readable report, Markdown reader view, JSON Schemas, and recursive tamper-evident manifest. Keep the audit additive and preserve all original Task `260` files and conclusions as historical evidence.
      - _Verify: deterministic mocked source/retrieval/statistical/tamper tests; opt-in live smoke against the real Task `260` v2 package and authoritative source pages; two-clean-interpreter replay where applicable; full paper banned-vocabulary and em-dash scan; Vault/Problem/Agent updates; and one focused commit._
      - _Outcome: completed as an additive publication-blocking audit over immutable Task `260` v2. Twenty-one primary sources across five adversarial perspectives were retained with 10 peer-reviewed, 8 preprint, and 3 normative/policy records. All three deterministic seeds duplicate scientific outputs within every mode/task, so the independent analysis uses ten tasks: mean difference `0.5`, frozen 20,000-resample interval `[0.2, 0.8]`, exact sign test 5 wins/0 losses/5 ties with one-sided `p=0.03125` and two-sided `p=0.0625`, UCI/MDBench means `0.25/0.666667`, and family-balanced mean `0.458333`. Three critical, 28 major, and 5 minor findings yield readiness `3/10` and verdict `major-revision-new-independent-evidence-and-human-review-required`. The defensible center claim is narrowed to a tamper-evident, failure-linked research state machine with auditable negative-result and claim provenance; Tasks `263.7.1`—`263.7.7` separate existing-evidence repair from new independent evidence and human decisions. Publication, public release, and external submission remain false._
      - _Verify result: eight deterministic unit tests passed. The opt-in live smoke retained and marker-verified all 21 authoritative pages, bound the real immutable parent, and reproduced projection `4247521dab59e0a65318f8391367aa11c26323d04335697be3e1f74f322f9cba` in two distinct clean Python installations. Report, source-registry, independent-unit-audit, replay, repair-plan, and manifest hashes are `92a478ee85f2324353f5310425408fb60d5c58fc2ee222b16069cbcdc1bfa190`, `50fbd19ad2a03896988ffa2d66d5b6499cf30c9996e9613a26c1cc4e97067427`, `b6a6e2cb59be88ebb4dc747a8c6d36d91a2279568a3c2cde711ac12acb751eb3`, `de0273ff820b898a58afc3689d5d524c9f7f8b1185a7d0e5cc4a84605416d253`, `4ad117a02defc318646456a9a754e91159756b5f148ae01f36f8ed1ddf36b3ec`, and `8e2dd7b5cbee5aa4274b125bc9f7c2cdab3ef33017a38f37e782ea35d089b9c9`. Full regression passed with 1,136 tests and 34 opt-in skips at 82% coverage; repository-wide Ruff and Mypy across 178 source files passed; Poetry validation, three Vault-link tests, recursive package reload, parent hash parity, and `git diff --check` passed. The paper scan found 28 restricted-language hits and no em dash._
    - [x] 263.7.1 Publish an additive independent-task reanalysis note
      - Preserve Task `260` v2 byte-for-byte, collapse the three deterministic seeds to ten task units, reproduce the frozen 20,000-resample task bootstrap, exact sign test, and family sensitivity, and label the original 30-cell interval as historical rather than publication-facing inference.
      - Bind every table, number, and prose claim to the immutable parent and Task `263.7.0` audit; reject any attempt to silently replace the original preregistration or convert idempotent seed replays into independent observations.
      - _Verify: exact task-vector and two-interpreter statistical replay, manuscript/table claim binding, original-package hash parity, schemas/manifest/tamper checks, Vault/Problem/Agent updates, and one focused commit._
      - _Outcome: completed as a separate post-audit unit correction over the byte-identical Task `260` v2 parent. The package binds the Task `263.7.0` commit/report/manifest/unit/source/repair evidence, inventories all 8 original claims, all 138 numeric leaves, both original tables, and 28 unit-sensitive LaTeX lines with zero unbound surfaces. Eight publication-facing 30-cell surfaces are retired; only C2 loses publication-inference status, while C1 is relabeled to the task unit and the remaining claims stay within engineering, negative-evidence, controlled-ablation, or scope boundaries. Nine additive claims report the ten-task vector, mean `0.5`, 20,000-resample interval `[0.2,0.8]`, exact sign test 5/0/5 with one-sided/two-sided `p=0.03125/0.0625`, and UCI/MDBench/family-balanced means `0.25/0.666667/0.458333`. The historical 30-cell interval is retained only as an internal-gate record. The note is not fresh confirmation, does not replace the preregistration, and leaves publication, public release, and external submission false._
      - _Verify result: eight deterministic unit tests and the opt-in real-parent smoke passed. The smoke bound both immutable packages, reproduced projection `4247521dab59e0a65318f8391367aa11c26323d04335697be3e1f74f322f9cba` in two distinct clean Python installations, reloaded the recursive manifest, and verified parent/audit parity. Report, manifest, audit-binding, surface-inventory, claim-ledger, unit-audit, and replay hashes are `476b920607ad981a1f0d7b0a33ff4d74e813a70159959c70386e9e15d6c37d99`, `f6d8371c9b1c54cb5ffa885c407210b74ede4b0c74d45466c6a2e074d089a6ab`, `c014c97241acb808f0a6de090180be33ba9cf3a54cc3eae16c66fe9ee85052d9`, `7ea653abaf1c3c7d3619ef7167161aee05badf6d847aae7d02a8a6950e23597e`, `f1f5bc960b159f6ede3cfb719e8590fd3ee77f2f50ec6d98df58d847207d4e41`, `b6a6e2cb59be88ebb4dc747a8c6d36d91a2279568a3c2cde711ac12acb751eb3`, and `de0273ff820b898a58afc3689d5d524c9f7f8b1185a7d0e5cc4a84605416d253`. Full regression passed with 1,144 tests and 35 opt-in skips at 82-percent coverage; repository-wide Ruff, Mypy across 179 source files, Poetry validation, Vault-link checks, and `git diff --check` passed._
    - [x] 263.7.2 Reposition the manuscript against the current autonomous-science field
      - Rewrite the title, abstract, introduction, related work, results, discussion, conclusion, and limitations so the frozen study is a controlled evidence-state-machine demonstration rather than a generic autonomous-research superiority claim.
      - Cover current peer-reviewed systems, clearly label preprints and independent audits, remove self-certified positioning tables, resolve all banned-language findings, and keep target venue, authorship, license, AI disclosure, release, and submission human-owned.
      - _Verify: retrieval-grounded claim/citation audit, full language/LaTeX scan, source-status checks, PDF build and visual inspection, unresolved-finding gate, Vault/Problem/Agent updates, and one focused commit._
      - _Outcome: completed as an additive current-field manuscript package over byte-preserved Task `260`, Task `263.7.0`, and Task `263.7.1` evidence. The new title is “A Tamper-Evident Research State Machine for Failure-Linked Computational Studies: A Controlled Local Demonstration”; generic autonomous-research superiority and the self-certified positioning table are removed. All 28 affected surfaces resolve, the historical 30-cell interval is restricted to a labelled internal-gate record, and the ten-task analysis remains a post-audit unit correction rather than confirmation. The manuscript resolves 29 citation keys and 37 occurrences; 17 current content-addressed snapshots are explicitly classified as 9 peer-reviewed, 5 preprint, and 3 normative sources. Full `.tex/.bib` scans find zero restricted-language hits and zero em dashes. The deterministic 10-page PDF contains seven vector figures and passes all-page visual inspection. Rewrite findings are empty and `rewrite_gate_passed=true`, while independent confirmation, human review, venue, authorship, license, AI disclosure, public release, external submission, and `publication_ready` remain false._
      - _Verify result: eight deterministic unit tests and the opt-in real-parent smoke passed, including citation/source-status exactness, complete claim/surface disposition, language/LaTeX gates, visual-review fail-closed behavior, partial-output rejection, recursive manifest validation, and persisted-artifact tamper rejection. Report, manifest, PDF, citation-registry, surface-ledger, and visual-review hashes are `0182c044157b293e69227118a40431fd8fa2d36be23dbf4556569fb135708a31`, `83baa8a732560facc6d6401fbc3ef0d87c3958ef45d56de447d72b32c8a7b6df`, `b7f6ed4a403b97f226aef2cf1604cde6dea038e82bf8dd0455bba0b46b41b0dc`, `9abce4eb9e710f7d44e2b151cbc360dccd65e8773a31a48027a080862c378230`, `0f157124a75eefb898a2d042a7e1ec6c416fed787decba147264251980468b0c`, and `649832e13cefa0e2b430d89033a9f39c497497fa8eeafa3e6ddb520382c23be1`. Full regression passed with 1,152 tests, 36 opt-in skips, and 82-percent coverage; repository-wide Ruff and Mypy across 180 source files passed; Poetry validation, three Vault-link tests, package reload, and visual/PDF checks passed._
    - [x] 263.7.3 Add an interoperable Open Science overlay to the immutable Task 260 object
      - Map the existing package to RO-Crate 1.3, Workflow Run RO-Crate, and PROV-O with explicit software, input, output, action, rights, identifier, claim-evidence, contradiction, limitation, and negative-result lineage while retaining all original hashes.
      - Provide machine-actionable internal/review views and exact reconstruction checks; do not treat metadata interoperability as independent scientific confirmation.
      - _Verify: profile validation, clean-directory reconstruction, recursive hash and provenance-query checks, original-package parity, sensitive/rights gates, Vault/Problem/Agent updates, and one focused commit._
      - _Outcome: completed as an additive, unreleased overlay over byte-identical Task `260`, Task `263.7.0` audit, Task `263.7.1` unit correction, and Task `263.7.2` rewrite. The internal view retains all 3,272 parent files; the review view exposes 22 sanitized, explicitly review-only artifacts; six authoritative RO-Crate/Workflow Run/PROV-O/validator pages are marker-verified and content-addressed, with raw bodies retained only in the local formal package under reference-only rights. A provenance-v2/PROV-O chain explicitly represents software, inputs, outputs, five historical/current actions, the controlled-demonstration claim, the retired 30-cell interpretation with counterevidence, the external-validity limitation, two Route A negative-result entities, and the publication-blocking decision. Seven clean-directory assertions and exact full-tree reconstruction pass. No public view, experiment rerun, independent confirmation, release, or submission is created._
      - _Verify result: four deterministic tests and one opt-in real-source/profile smoke passed. The formal package contains 3,371 recursively hashed files; all 3,272 Task `260` files reconstruct byte-for-byte, including the 3,269-file historical manifest plus its three excluded package-control files. `roc-validator==0.11.3` passed all required checks for Workflow RO-Crate 1.0 and Process/Workflow/Provenance Run Crate 0.5 on both internal and review views (8 reports); its bundled base profiles still stop at RO-Crate 1.2, so the external 1.3 availability remains truthfully false while the exporter validates the official 1.3 contract. Canonical self-excluding contract hashes for the report, manifest, provenance bundle, source registry, query, reconstruction, and profile report are `58199dc384adde6753548f7f9667c68e8d8ca2005341e808a2b5c5dda002c4c8`, `8e26b4de58ade45ee3a82bc9aef1e79d0e36d4c2fafd364c7bdf9395ee3a6d51`, `9fe90ec45ec1cdd5339a09939c36f1c461f11ca0d113e5ce13e638df35960a5c`, `ceb93e3163aa83571749ffffa8716bdb9dc22c9317098075f3eb80a6e8d807f4`, `851410e449cdaa8ed87cfc336f18ee6b349f0fd6ae411d3fde261958414b02b2`, `9cfa2879bded0c0768f46f336d1f98dd8e45ab43f3d85dd41c0be4f7a850100a`, and `f7229f5460a46e32e1d43299f00c3e9d26b998a6a5722a98aaf690beec0d0084`; the recursive manifest separately records on-disk byte hashes._
    - [ ] 263.7.4 Complete and bind the independent human benchmark-validity census
      - Depend on Task `263.6.7.3`: two real independent reviewers and a distinct adjudicator must complete the frozen search, dual coding, agreement, coverage, and at-least-20-family gates before any benchmark is selected for confirmation.
      - Keep the field mapping descriptive; retain unknown, conflict, unreachable, and excluded records, and stop at the registered diagnostic-negative endpoint when any frozen census gate fails.
      - _Verify: completed Task `263.6.7.3` human receipts/search/dedup/cards/agreement/coverage artifacts, exact synthesis replay, audit binding, Vault/Problem/Agent updates, and one focused commit or explicit human-gated stop._
    - [ ] 263.7.5 Freeze an independently authored systems confirmation protocol
      - Before outcome access, freeze independent task authors, at least three substantive task families, a justified sampling frame, compute-matched external research agents and simple baselines, null controls, task-level estimand, prospective power, family sensitivity, stopping rules, independent scorer, costs, and one-use confirmation permissions.
      - Separate task construction, AutoResearch development, baseline operation, scoring, and final scientific review roles. Existing Task `260` tasks may serve only as development history, never as the fresh confirmation sample.
      - _Verify: result-free preregistration and role/split/power/cost contracts, leakage and pseudoreplication tests, two-interpreter replay, human approval boundary, Vault/Problem/Agent updates, and one focused commit._
    - [ ] 263.7.6 Execute the one-use independent systems confirmation
      - Run the frozen task-by-system matrix once under common budgets, retain every failure/exclusion/cost/trajectory, keep task as the primary independent unit, and reproduce the analysis in an isolated environment without changing tasks, endpoints, baselines, or stopping rules.
      - Permit an adequately powered diagnostic negative. Do not create a favorable claim by benchmark shopping, dropping families, adding retries as units, or replacing external baselines after reveal.
      - _Verify: complete one-use matrix, null/control and cost checks, exact primary/sensitivity analysis, independent clean-room replay, claim-evidence package, Vault/Problem/Agent updates, and one focused commit or registered negative stop._
    - [ ] 263.7.7 Obtain an independent human publication decision and finalize the research object
      - Require two qualified independent scientific reviews plus conflict-resolved adjudication of novelty, validity, claim scope, ethics, authorship, licenses, AI disclosure, target venue, artifact policy, and public-release risk.
      - Authorize a submission-ready package only when every non-compensating gate is closed. Keep public release and external submission as separate explicit human approvals; automation may prepare but never issue them.
      - _Verify: human review/adjudication receipts, target-venue and policy audit, final claim/citation/reproduction/PDF/Open Science gates, explicit release/submission approvals, Vault/Problem/Agent updates, and one focused commit._
...
- [ ] 265. Build the formal autonomous competition research loop
  ...
  - [x] 265.2 Implement the provider-neutral literature-to-code branch engine and bounded Harness
...
- [ ] 266. Recover the autonomous competition loop after the Task 265.3 scientific-contract negative
  - [x] 266.1 Freeze a result-blind fit/freeze/predict recovery protocol
...
- [ ] 267. Repair the self-loop so it can autonomously reach a publishable outcome
  - Context: a repository audit plus a 2024-2026 literature cross-check established four independent root causes for the current all-negative output. (a) The Harness prompt contract advertises `term_count` and `factor_count` while the runner whitelist accepts only `{target, intercept, terms}` and `{field, derivative_axes, power}`, so a schema-compliant model answer is rejected as a scientific failure; runs `task2662-scientific-contract-harness-v1`..`v9` all ended with `passed_sentinel_count=0/6`, `fit_call_count=0`, and `predict_call_count=0`. (b) The configured provider rejects strict `json_schema` response format (`HTTP 400: This response_format type is unavailable now`), so the contradictory prose schema became the only instruction channel. (c) The `266.3` gate demands a >=5-percent gain plus a positive bootstrap lower bound in both the ODE and the four-system PDE stratum, which the project's own `266.1` power audit already declared underpowered for standalone significance. (d) The publishability definition is bound exclusively to beating a tuned SOTA symbolic regressor, so every honest run terminates as a negative result.
  - _References: `arXiv:2607.04108` "Dictionaries, Not Darwin" (parent-conditioned LLM evolution is statistically indistinguishable from independent sampling under matched call budgets; set-level selection over a term dictionary reaches 73.2-77.0 percent Acc0.1 on LLM-SRBench versus 49.2 percent for the best reported baseline at one tenth the call budget); `arXiv:2605.29184` IGSR (scalar-metric feedback cannot attribute per-term credit; granular influence scores plus MCTS repair this); `arXiv:2607.13608` MEDA (knowledge-guided formalization and mechanistic constraints are load-bearing; numerical fitting alone retains trajectory-compatible but scientifically wrong equations); `arXiv:2601.03315` "Why LLMs Aren't Scientists Yet" (six recurring failure modes, including weak experimental-design taste and success declared despite obvious failure); tasks `265.2`, `265.3`, `266.1`, `266.2`, `266.3`._
  - [ ] 267.1 Make the equation contract a single machine-checkable source of truth
    - Derive the prompt-visible equation, term, and factor key lists from the same exact-key constants the runner validator enforces, so the advertised schema and the accepted schema cannot drift again. Remove `term_count` and `factor_count` from the advertised contract and state the exact allowed key sets explicitly.
    - Add a contract-parity test that fails whenever the advertised key sets and the runner whitelist diverge, including nested factor keys.
    - _Verify: a parity unit test that fails against the current contradictory contract and passes after the fix, a replay of the exact `task2662-...-v9` `revision-03` candidate payload shape showing the previously fatal keys are no longer advertised, focused ruff and mypy, and one focused commit._
  - [ ] 267.2 Separate contract-format failures from scientific failures in the revision budget
    - Classify a schema or transport `ContractError` as a technical failure that does not consume the bounded model-only scientific revision budget, and return the exact offending key path and the exact allowed key set to the model as repair feedback. Keep genuine scientific failures (zero-null equivalence, absent training dependence, equation/prediction disagreement, leakage, shuffle non-degradation) fully budget-consuming and fail-closed.
    - Preserve every failure in the ledger and keep the technical/scientific distinction explicit in the persisted verdict so an audit can prove no scientific failure was reclassified away.
    - _Verify: deterministic tests proving a format-only failure does not decrement the scientific budget, a scientific failure still does, feedback carries the exact key path, immutability of prior packages, focused ruff and mypy, and one focused commit._
  - [ ] 267.3 Unify the provider on Qwen with negotiated structured output
    - Move `deployment.llm` to the Qwen DashScope OpenAI-compatible endpoint with `qwen3-max` for candidate/hypothesis authoring and `qwen3-coder-plus` for exact-code authoring, keeping `base_url`, `api_key`, and `model_name` provider-neutral and read from configuration or environment. Do not hard-code a vendor in engine code.
    - Negotiate structured output once per provider: attempt strict `json_schema`, fall back to `json_object` with local strict validation, and record which mode was used as provenance. Never treat a fallback as a strict-schema guarantee.
    - _Verify: mocked provider-neutral tests for both structured-output modes, an opt-in live DashScope smoke using the user-supplied `.env`, recorded negotiation provenance with no API-key value logged, focused ruff and mypy, and one focused commit._
  - [ ] 267.3.1 Repair Qwen reasoning-chain adaptation and make reasoning an explicit recorded output
    - Context: `llm/client.py` sent `{"thinking": {"type": ...}}`, which is an Anthropic-shaped parameter. DashScope silently ignores it and returns HTTP 200 with an empty `reasoning_content`, so the defect never surfaced across nine runs and 348 cells. A live three-way probe on `qwen3-max` with one identical prompt returned `reasoning_content` lengths of `0` for the Anthropic form, `301` for `enable_thinking=true`, and `0` for no parameter. The scientific-contract Harness additionally passed no thinking parameter at all, so exact-code authoring -- the step that most needs reasoning -- always ran with reasoning disabled.
    - Send provider-correct reasoning parameters behind a provider-neutral interface: DashScope receives `enable_thinking` plus a bounded `thinking_budget`; the Anthropic-shaped field stays reserved for Anthropic-shaped providers. Never hard-code a vendor in engine code.
    - Always bound the reasoning budget. Unbounded reasoning on `qwen3-max` produced 81,920 reasoning characters and 81,933 completion tokens for a trivial arithmetic prompt with an intermittently empty `content`; a bounded budget returned content reliably on `qwen3-max`, `qwen3.7-max`, and `qwen3.5-plus`. This is the same budget-exhaustion failure already documented for local Ollama Qwen 3.5.
    - Handle the structured-output interaction explicitly: enabling reasoning downgrades `json_schema` to `json_object` on DashScope and then requires the literal word `json` in the messages, otherwise the request fails with `invalid_parameter_error`. Use strict `json_schema` when reasoning is off and `json_object` plus local strict validation against the Task `267.1` single source of truth when reasoning is on. Record which combination was used as provenance.
    - Make reasoning an explicit first-class recorded output rather than a hidden side effect: persist `reasoning_content` into the interaction record alongside the existing explicit OPHIS `observation`/`problem`/`hypothesis`/`intervention`/`expected_effect` fields. Reasoning text is process evidence about how a candidate was authored; it is never scientific evidence for a result and must never satisfy an evidence gate or a publication claim.
    - Treat "reasoning enabled" as a preregistered comparison variable, not an assumed improvement. A 3-seed neutral-brief probe was directionally consistent with fewer query-only time-difference candidates but also produced one candidate with no real training fit, which is far too small to claim an effect. Task `267.6` Route P2 must measure it under matched call budgets.
    - _Verify: a unit test proving the DashScope path emits `enable_thinking` and a bounded `thinking_budget` and never the Anthropic-shaped field; a test proving the Anthropic-shaped provider path is unchanged; a test proving `json_object` plus local strict validation is used when reasoning is on and that messages contain the required `json` token; a test proving `reasoning_content` is persisted and flagged as non-evidence; an opt-in live DashScope smoke recording a non-zero reasoning length with non-empty content; focused ruff and mypy; and one focused commit._
  - [ ] 267.4 Insert a blocking research-plan confirmation gate into the autonomous loop
    - Require the system to generate an execution-ready research plan (hypothesis, mechanism families, panel, baselines, statistical estimand, stop rules, budget) into the vault and `outputs/<project-id>/research-plan/`, then physically block experiment execution until a human records an approve, reject, or revise decision. Wire the existing `ApprovalRecord` and `research-plan-audit` path into the campaign and competition loops instead of leaving `approval_required_permission_ids` empty.
    - A rejection or revision must return control to plan generation without consuming the scientific search budget, and the approved plan hash must be bound into every downstream execution record. Human approval of a plan is a scope decision, not a scientific result, and must never be counted as evidence.
    - _Verify: tests proving execution is impossible without an approval record, that the approved plan hash is bound downstream, that revise returns to planning, that approval is excluded from evidence counts, an end-to-end CLI walkthrough of the gate, focused ruff and mypy, and one focused commit._
  - [ ] 267.5 Replace the falsified search paradigm with dictionary-plus-set-level selection
    - Replace parent-conditioned multi-generation evolution as the primary search with a one-generation independent-proposal stage that extracts a per-problem dictionary of reusable candidate terms, followed by train-only set-level sparse selection with least-squares coefficients. Retain the evolutionary arm only as a preregistered matched-budget comparator, not as the default engine.
    - Upgrade candidate feedback from scalar NMSE to per-term granular influence scores, and add mechanism-constraint checks that reject trajectory-compatible but scientifically invalid equations, including zero-null-equivalent fits.
    - _Verify: known-law sentinel recovery under the new selector, matched-call-budget comparison against the evolutionary arm, per-term influence determinism, zero-null and mechanism-constraint rejection tests, focused ruff and mypy, and one focused commit._
  - [ ] 267.6 Preregister a dual publishable-outcome definition that admits an honest negative
    - Keep Route P1 as the original method claim under the unchanged >=5-percent and positive-lower-bound confirmation gate. Add Route P2 whose primary result is a preregistered controlled audit of LLM-evolution versus independent-sampling-plus-set-level-selection under matched call budgets, with the effect and its uncertainty as the reported outcome so that a well-powered negative is a valid publishable result rather than a loop failure.
    - Freeze the Route P2 estimand, panel, matched budget, sample size, and stop rule before any Route P2 score is observed. Report ODE and PDE strata separately, keep the four-system PDE stratum as a directional qualification only, and forbid substituting one route's result for the other after observation.
    - _Verify: a hash-bound dual-route preregistration, a power audit for the Route P2 estimand, result-blind construction proof, route-substitution rejection tests, Vault/Problem/Agent updates, and one focused commit._
  - [ ] 267.7 Run the repaired loop end to end and let the system author its own outcome
    - Execute the repaired loop under the confirmed research plan: literature pass, candidate authoring, plan confirmation gate, bounded search, selection, confirmation adjudication, and system-generated manuscript plus reproducibility package from the same append-only ledger.
    - Every scientific claim, number, figure, and citation must resolve to executed evidence in the ledger. Human-authored scientific prose and any unresolved number are rejected. Report whichever route qualifies, including an honest negative, and keep external submission a separate explicit human decision.
    - _Verify: claim-to-evidence and citation checks, figure/table consistency, ledger-origin audit, independent rebuild, zero post-start human scientific decisions, the recorded plan-approval decision, Vault/Problem/Agent updates, and one focused commit._

- [ ] 268. Repair pinned PDE baseline coverage so the frozen gate is reachable at all
  - Context: a pre-execution audit of `task2663-conformant-v1` found a blocker that outranks the zero-term defect. The frozen `266.1` estimand carries `all_domain_baseline_cells_must_succeed` as an immutable `Literal[True]`, and the pinned PDE baseline succeeds on only `72/84` baseline cells. `heat_laser` fails `0/6` with `index 3 is out of bounds for axis 0 with size 3` and `heat_soil_uniform_2d_p1` fails `0/6` with `SympifyError: None`, both raised as `every frozen sparse configuration failed` from `_run_sparse_baseline`. While those 12 cells fail, that frozen check can never pass, so no lineage can earn a receipt no matter how good its candidate is. The zero-term defect `P-20260802-068` is real but is not the binding constraint.
  - Context: the same audit showed the PDE stratum is far thinner than "four systems" suggests. Only `navier_stokes_cylinder` and `reaction_diffusion_cylinder` are paired at all; the other two PDE systems are excluded as baseline-coverage gaps under `P-20260802-065`. The reported PDE median `-15.402305` is therefore the mean of exactly two numbers, `-1.2891` from a real pair and `-29.5155` from the zero-term failure loss. Repairing baseline coverage restores the panel from 12 to 14 paired systems and the PDE stratum from 2 to 4, which is the only legitimate way to add power on the official panel without touching a frozen threshold or the sealed confirmation panel.
  - Context: this repair is expected to LOWER the measured effect, not raise it. The candidate's own loss on `heat_laser` is `15.46`, so once that system has a working baseline it will most likely contribute a negative effect. This task is a correctness prerequisite that makes the gate evaluable and the estimand honest; it is explicitly not a route to a favourable number, and it must be implemented score-blind.
  - _References: `P-20260802-065` unpaired-baseline exclusion; `P-20260802-066` cross-stage spend ledger; `P-20260802-067` shape-derived term cap; `P-20260802-068` zero-term selection; `P-20260802-069` generation enforcement; frozen Task `266.1` plan, domain baseline policy, and estimand; `266.1.1` as the precedent for an immutable-parent erratum._
  - [x] 268.1 Diagnose both pinned PDE baseline failures against the real payloads
    - Characterise each failure exactly before changing anything. For `heat_laser`, establish how the `(201, 201, 3, 20, 1)` payload is passed to `set_spatial_grid` and finite differencing, and which axis the out-of-bounds index refers to. For `heat_soil_uniform_2d_p1`, establish which stage returns `None` where an expression is expected so `sympify` receives it.
    - Record whether each fault is in the adapter's shape transport or in the pinned baseline library itself, because only the former is ours to repair. Do not tune any baseline hyperparameter, and do not alter the frozen configuration grid produced by `_baseline_configs`.
    - _Verify: a read-only reproduction of both failures from the retained conformant baseline cells, an exact statement of the faulting axis and the `None`-producing stage, focused ruff and mypy, and one focused commit._
  - [x] 268.2 Route the repair decision into the system's own self-correction cycle
    - Re-scoped before execution, and the re-scope is itself a finding. The original body demanded an adapter repair plus a re-pinned baseline image, and its verify line demanded that "all four PDE systems produce a real finite loss under the repaired runner". The `268.1` evidence proves that goal is UNREACHABLE under the frozen policy rather than merely unimplemented: `heat_laser`'s `z` axis physically carries 3 samples while the frozen `pde_derivative_order=[2]` needs `d + order = 4`, and `268.1` also established that NEITHER failure is in our shape transport, so there is no shape-transport defect for this task to fix. A task cannot be verified against an unsatisfiable condition, so the deliverable is the DECISION about that contradiction, not an execution of it.
    - The repair route is a scientific decision, so it must originate inside the loop instead of being chosen by an agent. Observe the contradiction deterministically from the retained conformant baseline evidence, classify each failing system's mechanism, ask the configured model to author the repair itself, then audit that proposal against the same evidence with a guard that refuses any route which would fabricate an effect or weaken the baseline.
    - Author a proposal only. Do not execute the repair, do not re-pin the container image, do not mutate any retained artifact, and do not spend candidate budget. A proposal is never an authorization: execution stays gated behind human plan approval and a new preregistration lineage.
    - Bounded reasoning must actually be engaged for this authoring call, since it is the most reasoning-heavy step in the loop. Thread the reasoning parameter through the shared authoring transport once for every caller rather than special-casing this module, keep the budget bounded, and record `reasoning_content` as explicitly NON-evidence process provenance.
  ...
  - [ ] 268.4 Restate the conformant lineage's adjudication honestly under the new baseline policy
    ...
    - Report whether the overall median, bootstrap interval, and both strata move up or down, and state plainly that a downward move is the expected and acceptable outcome of removing an artificial exclusion. Do not average results across two baseline policies inside one effect.
    - _Verify: the restatement reuses the retained candidate cells with zero new candidate spend, the prior package and cells remain hash-identical, the paired and unpaired system sets are named explicitly under the new policy, no effect mixes two baseline policies, both strata reported separately, `search_freeze_receipt` still false, confirmation still sealed, and `Problem.md` plus `Agent.md` updated._
  - [x] 268.5 Thread bounded reasoning through the shared autonomous authoring transport
    - `P-20260803-071`: `_call_and_record` is the single authoring transport for every autonomous path (candidate portfolios, implementations, technical repairs, Route P2 self-correction, and the frozen-protocol repair cycle), and it hard-coded `thinking_mode="disabled"` both in the provider call and in the persisted record. The most reasoning-heavy steps in the system therefore ran with the reasoning chain OFF while recording that fact accurately, which is the silent no-op defect class of `P-20260802-050`. Thread the parameter through that helper ONCE for every caller instead of special-casing any module, and do not add a second transport path.
    - Keep the budget bounded. `P-20260802-051` recorded that unbounded reasoning on this provider family returned 81,920 reasoning characters with an intermittently EMPTY `content`, losing the answer, so an enabled call must carry an explicit bounded budget and the record must reflect what was ACTUALLY sent rather than a constant. Reuse the provider-neutral dispatcher from Task `267.3.1`; do not duplicate it.
    - Handle the structured-output interaction honestly: enabling reasoning downgrades transport-level `json_schema` to `json_object` on DashScope-shaped providers, and `json_object` mode requires the literal lowercase word `json` in the messages or the provider rejects with `invalid_parameter_error`. A reasoning call therefore goes straight to json-object mode with LOCAL strict validation, which is a deliberate request shape and must not be recorded as a `response_format` provider fallback.
    - Persist `reasoning_content` as explicitly NON-evidence process provenance that can never satisfy an evidence gate, a metric claim, or a publication claim, and never log a secret value. Adding these fields changes FUTURE interaction hashes, which is acceptable only because no lineage is mid-flight; a reasoning-free interaction must still serialize byte-identically to the pre-`268.5` shape so no RETAINED evidence is retroactively invalidated, and no retained interaction may ever be rewritten.
...
- [ ] 269. Run a fresh preregistered lineage targeting the diagnosed zero-term selection
  ...
  - [x] 269.5 Repair the complementary PDE defects in a further preregistered lineage
    - _Outcome: COMPLETED across two further lineages, and it resolved into a scientific conclusion rather than an engineering one. `task2695-pde-repair-lineage-v1` carried the parent's real failure reasons and the loop DID improve on what it was told: two candidates reached `10/10` in the pilot against a previous best of `8/10`. But its full stage exposed `P-20260804-082`, a defect in the promotion gate I had added: `_split_smoke_wave` keyed on `candidate_id` alone, so the smoke wave covered only the first system, which is always ODE in the frozen spec order. A candidate that exceeds the PDE wall-time budget therefore passed its smoke wave and failed all 12 PDE cells. The wave now takes one representative system per `(candidate, data_type)` pair, and promotion deliberately stays ALL-OR-NOTHING rather than narrowing to the passing stratum, because refusing only the failing stratum would let a candidate dodge the systems it loses on._
    - _Outcome (`task2696-stratified-gate-lineage-v1`, the first fully clean measurement): the stratified gate worked exactly as intended on real cells, reporting `official-03-r2 [ode 0/6, pde 6/6]` and `official-05-r2 [ode 6/6, pde 0/6]` with explicit warnings before the remaining cells ran, which is precisely what v3's gate could not see. `official-07-r2` then achieved **`72/72`**, so `all_candidate_cells_succeeded` **PASS** together with `all_baseline_cells_succeeded` **PASS** for the first time in this line of work: `144/144` candidate cells and `72/72` baseline cells, zero infrastructure failures, zero zero-term failures. Baseline `72/72` has now reproduced across FOUR consecutive independent lineages. Package `fb49f72644e07192d8bbf1b7c43414f93d588c85ac466f35575b9ab096c338e5`._
    - _Outcome (the scientific result, reported as the negative it is): `search_freeze_receipt` **False**, and the reason is now the science rather than the code. `candidate_win_count` `3` of `12`, `overall_median_log_effect` `-0.8448548894388439`, bootstrap `[-1.4630988707200518, +0.04814249650803004]`, ODE stratum `-0.6556574227708623`, PDE stratum `-1.594291281789289`. The measurement was verified CLEAN before being reported: zero cells at the frozen failure loss, 19 distinct loss values from `0.0` to `257.02` with term counts `1..253` so it is a real fit rather than a constant predictor, and the shuffled-training control changed the artifact hash in `72/72` cells so the fit provably reads its training target. Recorded as `P-20260804-084`._
    - _Correction to my own earlier reading, recorded rather than quietly dropped: I first assumed the better ODE medians in `task2694` (`-0.0553`) and `task2695` (`-0.0452`) were failure-loss artifacts. They were not. Both fully executed all 10 ODE systems with zero failure-loss cells and each won 5 of 10. The real difference is a CAPABILITY TRADE-OFF: those medians came from candidates that could not run PDE at all, while `task2696`'s candidate runs both strata and is a weaker ODE fitter. Per-system it turns `driven-pendulum-quadratic-damping` from `-7.7902`/`-4.4554` into `+0.9320` and improves `population-growth-naive` from `-5.2943` to `-1.0323`, while losing `binocular-rivalry-model` from `+2.3550`/`+4.2566` to `-1.2160`. Different candidates win different systems and none dominates._
    - _Stopping decision, stated as a decision rather than an outcome: three independent candidates, each fully executing the same 10 ODE systems, produced ODE stratum medians of `-0.0553`, `-0.0452`, `-0.6557` and win counts of 5, 5, 3. That is a stable null. Drawing further lineages until a favourable number appears would be selecting the outcome, which is exactly what the frozen preregistration prevents and what `P-20260802-060` already records as an error made once. The honest report is the Task `267.6` Route P2 preregistered negative, with a tight interval that makes it informative rather than an underpowered shrug._
    - Blocked on a new lineage, because `task2694-promotion-gated-lineage-v1` spent both frozen generations (`2/2`) and the ledger refuses a third (`P-20260802-069`). The two structural repairs are now REPRODUCIBLE across two independent lineages, so this subtask inherits a working substrate rather than a broken one.
    - Feed each candidate its OWN PDE failure reasons through the score-blind self-revision channel: `official-08-r2` must repair an off-by-one grid axis (`IndexError: index 30 is out of bounds for axis 1 with size 30`, and the same at size 15) that fails every `clean`-condition PDE cell, and `official-03-r2` must repair an equation that names a state field the evaluator does not have for `reaction_diffusion_cylinder` (`ContractError: equation factor names an unknown state field`).
    - Do NOT special-case either system in the runner, do NOT repair the candidates' code by hand, and do NOT combine the two finalists. Their failures are complementary, so taking `navier_stokes_cylinder` from one and the `snr_20` cells from the other would fabricate a result neither candidate produced. The repair must be authored by the system.
    - The ODE stratum is already at `-0.0553167112006033`, effectively a tie with the pinned baseline, with `5` of `10` ODE systems won. Repairing the 6 PDE failures removes 6 frozen failure losses from the selected candidate's cells, which is the single largest lever on `overall_median_log_effect`, `bootstrap_lower`, and `all_candidate_cells_succeeded` simultaneously.
    - _References: `P-20260804-081` complementary PDE defects; `P-20260802-069` generation enforcement; `P-20260804-080` promotion gate; the `task2694` signed package `eaaab3746ae3cdaa3165d330585ab0aefcad64f0ef14cc5c22e0020232df2d9e`._
    - _Verify: every stage refused without an approval, the smoke wave gates promotion, zero zero-term failures, all 12 baseline systems succeed again, and the selected candidate's PDE cells either succeed or their failure is reported as a candidate-side defect rather than silently absorbed into the effect._
  - [ ] 269.4 Execute the gated pilot, self-revision, baseline, and full stages
    - BLOCKED on a human protocol decision, and the blocker is a finding rather than an obstacle. Pre-execution work found that the preregistered exclusion was DECORATIVE (`P-20260804-076`): `run_plan_stage` was policy-aware but `_stage_shape` returned the whole panel, so the baseline stage would still have executed both excluded systems, their 12 cells would still have failed, and `all_baseline_cells_succeeded` would still have been false. The new lineage would have burned the entire frozen budget and reproduced the parent's blocker while reporting a 12-system policy. That is fixed: `narrow_panel_by_policy` binds the declared panel change to the cells actually built.
    - Fixing it exposed a SECOND independent frozen contradiction (`P-20260804-077`). The panel carries exactly 4 PDE systems, the policy excludes 2, and the frozen budget requires `pilot_pde_system_count=3` with a derived `pilot_system_count=6`. Neither is reachable on the narrowed panel. Honestly repairing one contradiction therefore proved the frozen `266.1` protocol unsatisfiable on a second axis, which strengthens the `268.2` conclusion rather than contradicting it.
    - That decision was routed into the system's own loop, not taken by an agent. `pilot_breadth_contradiction.py` follows the `268.2` contract and offers a CLOSED set of four routes, unranked and including the dangerous ones. The guard classifies the route actually chosen rather than the route claimed, so a proposal that misreports its own risk is refused twice.
    ...
    - _References: `P-20260804-076` decorative exclusion; `P-20260804-077` frozen pilot breadth unsatisfiable; `P-20260804-075` the plan approval is an agent-signed scope authorization whose own terms exclude editing a frozen budget parameter; `P-20260804-078` provider transport errors; Task `268.2` self-correction contract._
    ...
    - _Resolved-blocker: the frozen pilot breadth was unsatisfiable on the narrowed panel and could not be rewritten in place. The system reached `declare_frozen_pilot_breadth_unsatisfiable_and_require_new_preregistration` in all THREE independent live runs with bounded reasoning enabled (`1587`, `1349`, `1144` real reasoning tokens), with `guard_accepted=true`, `edits_frozen_budget_parameter=false`, and `contaminates_finalist_selection=false` every time; only the justification wording varied. `preregistered_stage_breadth.py` executes that authored decision: the child lineage declares its OWN breadth (`0b555f6226a4a3f5`, 5 systems = 3 ODE + 2 PDE) while the frozen `266.1` budget stays byte-identical and is bound as evidence of what was superseded. This is the `266.1.1` immutable-parent pattern, NOT the `reduce_pilot_pde_breadth_to_available` route the guard refuses. Narrowing the pilot cannot bias the effect: `rank_pilot_finalists` ranks by validation NMSE only and no pilot cell enters the estimand, asserted by a permanent `pilot_enters_estimand: Literal[False]`._
    - _Outcome: the lineage RAN END TO END and produced a signed package, `807a9d9b4ae384147d210f2dfff8b62e2a3bba841cd0b7af21e0128b500d019c`. Report the two structural repairs and the scientific result separately, because they diverge. STRUCTURAL REPAIRS SUCCEEDED: `all_baseline_cells_succeeded` is **PASS** for the first time in this line of work (`72/72` versus the parent's `72/84`), `baseline_coverage_gap_count` is `0`, `paired_system_count` is `12`, and `zero-term failures` is `0` across every executed cell, which is the `P-20260802-068` defect Task `269` exists to fix. So the exclusion genuinely bound, and the frozen coverage check became satisfiable rather than merely being declared repaired. SCIENTIFIC RESULT IS A STRONG NEGATIVE AND IS DOMINATED BY CODE DEFECTS, NOT METHOD QUALITY: `candidate_win_count` `0`, overall median log effect `-27.779439711880524` with bootstrap `[-29.46571482355266, -24.869098342323543]`, ODE stratum `-26.57408181464895`, PDE stratum `-29.46571482355266`, so `search_freeze_receipt` is `False`. The cause is `P-20260804-080`: `official-05-r2` failed ALL 72 of its cells with one uniform `TypeError: can't multiply sequence by non-int of type 'numpy.float64'`, an unconditional crash, because a REVISED candidate reaches the full stage without ever having executed once. The pilot runs the pre-revision candidates. `official-02-r2` burned a further 61 cells on shape-concatenation errors, repeated-support contract violations, and 6 timeouts. This lineage's effect is therefore NOT a fair measurement of the method, and it must not be reported as one. Both frozen generations are spent (`2/2`) so the candidates cannot be repaired here. `assert_finalists_can_execute` now refuses to promote a finalist with no successful cell, and refuses the whole stage if no finalist can run; one smoke cell per finalist costs 2-3 cells against the 72 wasted here._
...
- [ ] 270. Close the submission-critical autonomy and evidence-integrity gaps
  ...
  - [x] 270.2 Bind approved research-plan scientific content into generation and execution
    ...
    - _References: Research Plan §29 and Execution Plan §29; `P-20260808-100`; Task `267.4` approval gate; immutable-evidence rule from `266.1.1`._
    - _Outcome: completed. Approved scientific fields are copied verbatim into hash-bound `plan-execution-contract-v1`; both generation passes receive the full contract in retained prompts; registries bind plan hash, contract hash, exact source hash, and a deterministic AST audit. Only called helpers reachable from `fit_equations`/`predict_derivative` can satisfy method tokens—comments, prose, variable names, and dead helpers fail. Generate and revise now require plan approval and contract compilation before `_freeze` or any provider call, while pilot/baseline/full reload the retained contract and reject hash-only candidates before starting a container. A read-only audit of `task2700` extracted `integral`, `bayesian`, `constrained`, and `lars` from its own plan and rejected all 11 generated/revised candidates (0/11 aligned), proving the retained result is not evidence for that plan. Broad competition regression: 649 passed with only the four already-recorded host-numpy cases deliberately deselected; focused ruff and mypy clean._
  - [x] 270.3 Materialize observed results into a final research report
    ...
    - _Outcome: completed. `competition mdbench final-report` now performs a 19-check fail-closed input audit, joins only the exact system-authored/approved plan, plan-execution contract, source-hash-matched aligned candidate, signed complete package, and accepted relation-audited outcome, then writes canonical JSON plus Chinese Markdown/TeX/PDF views and a hash-bound build receipt. It rereads and hashes both preregistration files before and after rendering, verifies the exact verdict/key metrics across all four formats, and never writes observed data into the plan. System-authored title, plan prose/lists, result interpretation, and limitations must independently pass a deterministic Chinese-language gate at input audit and final-report schema validation; technical identifiers and original bibliography metadata remain searchable. The retained `task2700` lineage is refused because it lacks a plan contract, has no plan-aligned selected candidate, and predates numeric-relation auditing. Real XeLaTeX/pdftotext compilation and five-page visual inspection passed; 75 focused tests and the broad competition regression (660 passed, only the four recorded host-NumPy cases deselected) verify the implementation._
  - [x] 270.4 Assemble a fail-closed submission evidence bundle
    ...
    - _Outcome: 已完成。`competition mdbench submission-audit` 现在输出一个规范化 JSON 证据包与一份中文 Markdown 总审计，覆盖 19 项必需检查。新计划和结果解释保存不含凭据值的精确模型调用回执；入选候选绑定真正被接受的生成或修复调用；每个原始单元都从冻结外层规格，经容器规格/结果哈希，连接到阶段汇总和签名结果包。审计还核验官方身份、预算账本、预注册基线/试运行宽度、完整的系统×条件×种子矩阵、候选选择/效果/自助区间/门禁重算、配置与记录的 provider/base URL/model 一致性、最终报告各格式、独立复跑、创新审计、凭据泄漏和严格 pytest/ruff/mypy 证据。缺失证明会成为明确阻断项，不会因异常而让报告消失。只有所有检查（含 `publication_readiness`）全部通过，schema 才允许 `submission_ready=true`；历史 `task2700` 谱系仍被正确阻断，且未被修改。_
  - [ ] 270.5 Promote system-authored Chinese preregistration into the official lineage
    - Replace the fixed English `official_plan_generation` path and ignored `_v7.py` scratch orchestration with one reviewed CLI command. It must freeze a fresh zero-spend lineage, preregister policy and stage breadth before child results, let the configured model choose literature queries from retained raw evidence, retain real retrieval results and exact model interactions, then let the model author every scientific plan field in Chinese.
    - The orchestration may supply immutable protocol and prior signed evidence only. It must not supply a human-authored hypothesis, mechanism, problem statement, method, expected result, title, or framing. It must persist the exact model-authorship receipt and stop with execution unauthorized until a human approves the exact generated plan hash.
    - _Verify: mocked deterministic tests prove raw-evidence-only context, Chinese literature relevance and model receipts; a live provider + ArXiv/OpenAlex smoke produces a Chinese plan with zero hand-written scientific prose; generate is refused before approval; focused pytest/ruff/mypy and one focused commit._
    - _Progress (blocked, 2026-08-09): the reviewed command, exact literature/ideation/review/plan receipts, per-field Chinese guards, result-blind numeric provenance, plan-to-method-token binding, adversarial full-plan review, safe resume path, and pre-approval execution refusal are implemented. A second independent selected-direction prosecutor now vetoes renamed standard diagnostics, invalid controls, non-identifiable mechanisms, arbitrary thresholds, infeasible statistics, and component composition before plan authoring. The focused regression is green (`137 passed`; focused Ruff/Mypy clean). Two fresh live provider + ArXiv/OpenAlex runs (`task2705-preregister-plan-smoke-v12` and `v13`) failed closed. In `v13`, four bounded portfolios produced 20 model-authored Chinese directions; four independent reviews rejected all 20 for prior-work equivalence, invalid scientific assumptions, non-identifiability, or frozen-budget/interface infeasibility. No `system-plan-ideation.json`, official research plan, approval, execution, or publication claim was created. The live-plan acceptance criterion is therefore NOT met; this task stays unchecked and must not be committed as complete._
    - _进展（继续受阻，2026-08-09）：证据边界现已包含实际公开开发数组的确定性画像（12 个可研究系统、24 个哈希绑定的 clean/noisy 数组路径）以及由模型单独撰写、独立评审的机会图。精确回执续跑、累计被拒方向记忆、目标白名单、科学签名/方法 token 防换皮和去锚定重试，阻止系统通过改名掩盖失败想法。真实 `v15` 自主生成 4 条查询，保留 59 条 ArXiv/OpenAlex 记录，以中文选择 10 篇文献，并产生 6 个有界方向续跑、47 份精确交互；没有任何一个写出 `system-plan-ideation.json` 或计划。更严格的回溯消费检查还拒绝了唯一曾暂时通过的 O04：其自身 `feasibility_risk` 错把 20 秒 synthetic-sentinel 预算用于正式单元，虽然评审随后改用了正确的 300 秒/4096 MB scope。新增作者侧校验现会在评审前阻断这类污染。聚焦套件全绿（`168 passed`；变更文件 Ruff 与 11 模块 Mypy 均 clean），但广泛质量门仍为红色（`729 passed, 4 failed`，另有 3 个既存 Ruff 和 5 个既存 Mypy 问题）。因此 `v15` 仅包含有价值的负过程证据，没有有效机会制品、科学计划或可发表结果；Task `270.5` 继续未勾选。_

- [ ] 271. Establish sovereign raw memory and rebuildable Obsidian Dreaming
  - [ ] 271.1 Add the local append-only raw-memory foundation
    - Retain authorized exact source bytes under a Git-ignored private-local root, with canonical metadata, exact byte/hash verification, exclusive creation, project isolation, append-only corrections, and fail-closed credential/direct-identifier scanning.
    - Keep mutable `MarkdownKnowledgeStore` entries, indexes, embeddings, and summaries outside the authoritative raw layer. Every Dreaming projection must bind exact raw record/payload hashes, declare itself derived and rebuildable, and retain epistemic labels for supported, contradicted, extrapolated, and unverified claims.
    - _Progress (2026-08-09): `RawMemoryStore` now keeps v2 payloads as opaque content-addressed blobs, verifies canonical record bytes and exact payload size/hash on every read, uses atomic create-once publication, rejects path/symlink escape, isolates projects, and writes only owner/SYSTEM-readable Windows ACLs (or owner-only POSIX permissions). Normal Obsidian indexing and backup explicitly exclude `_private`, while `.gitignore` excludes both `_private` and vault backups. Dreaming v2 binds exact local raw-record/payload hashes, uses an atomic commit marker, labels supported/contradicted/extrapolated/unverified claims, declares itself derived/rebuildable, and retains superseded views. Model-review raw responses are captured before quality rejection and parsed output is rechecked against the exact response. The supplied screenshot and official paper PDF are both retained: payload hashes `b33d1c2fe877912dbc1962759bf0c7d8321fd08c386aa0f406ff96df8d7aece4` and `4b239639242fc2ca1be467cf49ad70789c9467f9917c2869d1c2c712a8367e26`; the current Chinese Dreaming projection is `dream_f3460b5aa8bfab5b5b8960ca310c1c9c88d93561c295c39130962d1841b9f6b7`. Real-artifact verification plus 69 focused tests, Ruff, and Mypy pass. Completion remains open because user-key encrypted export/restore, capture of every model/tool transport attempt, stronger binary/OCR privacy review, subject-bound correction chains, broad regression, and a focused commit are not yet complete._
  - [ ] 271.2 Migrate source/model/tool ingestion and schedule governed Dreaming
    - Route literature bodies, user-authorized attachments, model transcripts, and tool outputs through the raw boundary before projection. Add explicit private-data approval/encryption rather than weakening the secret gate.
    - Run bounded nightly Dreaming over newly appended records, preserve transform/prompt/skill/model receipts, and prove delete-and-rebuild equivalence for derived views. Do not make any Dreaming output authoritative scientific evidence without the normal provenance and validation gates.
  - [ ] 271.3 Establish the adaptive sovereign research dual loop
    - Replace fixed inner-step orchestration with a generic stage main-agent controller. From one Chinese objective/scope seed, the configured model must choose the next operator among retrieval, hypothesis branching, analogical transfer, reframing, uncertainty decomposition, temporary-Agent consultation, bounded probes, adversarial critique, Dreaming, workflow-proposal mutation, promotion, abandonment, and stop. The orchestrator must not supply a hypothesis, method answer, expected result, or research plan.
    - Apply stage-local strictness: open exploration accepts unsourced but explicitly unverified hypotheses; promotion requires multiple traceable sources, a falsifier, decisive test, discriminating controls, external feedback, bounded resources, and independent verification; execution/publication remain separately human-authorized and fail closed. Workflow mutations may never alter safety, evidence, permission, license, or publication policy.
    - Route project methodology dynamically from `skills/*/SKILL.md`: Qwen first sees only metadata and may choose zero skills; exact selected bytes are then injected as separate read-only messages. Count routing and action calls in one budget and retain exact response/reasoning/raw-memory bindings plus immutable routing artifacts.
    - Connect the controller to sovereign raw memory and a rebuildable branch/event/snapshot archive. Temporary Agents must be assigned by the current stage main Agent, return content-only artifacts, be archived, and lose runtime identity before their output is consumed.
    - _Progress (2026-08-10, corrected after adversarial replay): the generic controller, 13 model-selectable operators, open/promotion/verification zones, branch archive, dynamic zero-or-more Skill router, capability adapters, rebuildable Dreaming, current-stage temporary-Qwen dispatch, and independent promotion verifier remain implemented. Action and model-backed Skill calls now receive immutable pre-call reservations in both ordinary output and sovereign raw memory; unresolved failures consume budget, exact external-context bytes/messages and typed event transitions are replayed, recall is restricted to exact snapshot membership, and Dreaming selection is deterministically rerun. The per-turn provider JSON Schema now exposes only the mechanically reachable operator subset and the identical schema hash is written before the call, fixing a live Qwen failure caused by the task listing eight operators while the schema still allowed thirteen. A fresh `qwen3.7-max` smoke then completed two autonomous turns from one Chinese objective/scope with no intermediate user research instruction: two dynamic Skill-routing calls plus `branch_hypothesis → adversarial_critique`, four model calls total, final snapshot hash `b1008c3e8967f05ba323571609765c7bb4f912c9b70382be1965897d433af47c`, and 15 verified sovereign raw records retained under the ignored live run. This is functional self-loop evidence only, not scientific correctness or a tamper-proof provider attestation. The invalid five-template/three-seed design has been replaced by protocol v3: 60 independent scenarios × four paired arms = 240 blinded cells, twelve public turns, a nine-turn memory gap, balanced order, exact-McNemar A4−A3 primary contrast, and a separately sealed hidden oracle; validation execution hash is `cfe042f2061f89e3d8a56d1a39fe65a056fd88ab11d7912165a926b67991d6a3`. The receipt bridge, four arm adapters, public-only context adapter and diagnostic single-cell runner now close four 12-turn local trajectories, budgets, journals and terminals without loading scoring data or claiming results. Red-team then proved that ordinary in-process `urlopen` and arbitrary-byte receipt builders could self-assert an external Qwen boundary, so all LLM/benchmark transport artifacts have been fail-closed to `process_local_only=true`, `boundary=false`, `formal=false`; self-reported Qwen/model/ID, HTTP status and injected or replaced openers cannot promote them, and old formal artifacts are rejected. Final focused gates include 21 loop tests, 56 LLM tests and 343 research tests plus Ruff/Mypy. Task completion remains open for an independently signed external transport gateway, a formal live cell runner, an exact A4 early-raw→Dreaming→later-request/terminal consumption receipt, a non-confirmatory 12-turn live pilot, all 240 cells, blinded scoring, and a focused commit. No causal memory benefit, superiority, innovation, formal execution or publication claim is authorized._
    - _Progress (2026-08-10, signed runner and twelve-turn live audit): the independent Ed25519 gateway v3, same-response completion, exact provider-model binding, local anti-replay semantics, formal-live runner, and `SovereignRecallUseReceipt` are now implemented and adversarially tested. The gateway/recall/formal linked suite passes, but a real signed pilot correctly stopped before its first event because the current VPN/TUN resolves the provider to non-global RFC 2544 address `198.18.0.35`; no IP/TLS/trust gate was weakened. A separate non-confirmatory live sequence exposed and fixed one generic Qwen repair contradiction: external-context raw bindings are no longer eligible as Dreaming claims, memory claims may only be conservatively retracted during repair, typography-only quote normalization is allowed, and substantive scientific rewriting remains forbidden. The fresh v4 behavior pilot then completed twelve `qwen3.7-max` actions from one frozen objective/scope with twelve one-shot calls, zero intermediate human/orchestrator scientific prose, minimum reasoning length 1842, and snapshot hash `8e05f684e813a53139b63eb5785ac5217a066a100c867a1d28942ee21ef39388`. Its model-selected trajectory was `decompose_uncertainty×4 → branch_hypothesis → adversarial_critique×4 → decompose_uncertainty×2 → promote_branch`; deterministic promotion correctly rejected untraceable/no-external-feedback evidence. This is strong functional self-loop evidence, but the same run selected Dreaming zero times, produced zero recall-selection artifacts and zero structured memory-consumption claims, so the live test correctly failed and `actual_sovereign_recall_use_verified=false`. The frozen v3 benchmark must not be modified after this result. A steerable-memory/anti-local-loop policy may proceed only as a new shadow-tested strategy candidate on frozen trajectories and fresh development scenarios, followed by a new preregistration if warranted. Task `271.3` remains unchecked; 240 cells, blind scoring, causal memory benefit, scientific correctness, innovation, formal external identity and publication are not established._
    - _Progress (2026-08-10, generic Qwen adaptation and fresh development v6): retrieval output was separated into model-authored structured short phrases instead of asking repair to infer machine queries from prose; the Skill router removed a redundant negative Boolean so an empty selected list is the sole no-Skill representation while retaining legacy replay; provider-unsupported `uniqueItems` was removed from the transport schema while deterministic duplicate rejection remains local. These are generic contract reductions, not scientific-answer injection. The fresh `task2713-adaptive-operator-steering-development-pilot-v6` then completed 15 actions from one frozen Chinese objective/scope, with 15 Skill-routing calls plus 15 action calls, minimum reasoning length 2731, two external actions, three main-Agent-dispatched temporary Qwen tasks, zero intermediate human/orchestrator scientific prose, and final paused snapshot `4871ac139be2775c69b066b78922cf47a975d7f7845b73896ed7d5cc5add3167`. Qwen autonomously moved from decomposition and critique to a distributed-cache analogy, branched, delegated three bounded content tasks, requested promotion, reacted to rejection with eight model-authored search phrases, abandoned the branch after negative retrieval, and created a new mechanism branch. The structural steering candidate saw no task text, benchmark, hidden oracle, paths, environment, time, random seed, or future trajectory; its only two interventions removed saturated introspection actions but left ten ordered choices and never forced Dreaming. This strengthens functional self-loop and non-special-case evidence. It does not establish sovereign memory use: Dreaming, out-of-window recall, and structured consumption remained zero, audit `3949648dff9be950f61d6be252c00e3080bbc2ba8c22df6ea4da3c6204a39b08` stayed negative, and the opt-in live test correctly failed its frozen memory-success criterion. Task `271.3` remains unchecked and the strategy remains shadow-only._
    - _Progress (2026-08-10, v8—v12 temporal-memory audit and protocol falsification): the Chinese action guard now scores bounded Latin word runs instead of individual letters, allowing identifiers while still rejecting English prose; only invalid Chinese action bodies may be reauthored by configured Qwen, with scientific identity and all other fields frozen. Retained workflow proposals are now temporally projected with authored step and age so old “this/next turn” prose cannot silently become a current instruction. Fresh v9/v10 runs each completed 15 autonomous actions and 30 model calls; v9 dispatched three temporary Qwen tasks and v10 performed retrieval, yet both correctly remained zero-Dreaming/zero-consumption. V11 failed before any provider byte because the live harness passed a function where the loop requires a `contexts_for_turn` provider; the strong adapter fix enabled v12. V12 completed 12 actions in 24m28s, 12 Skill routes and 27 total model calls, with `decompose/branch/critique → one autonomous Dreaming → reframe → stop`; final snapshot `24d2d173ea434f8704f5269906ec779cdcba9e5ed40febdbce0de30edf051a1d`. The Dreaming selection and exact feedback replayed into the next request, but all eight selected records were recent, the terminal action had no structured consumption claim, and memory audit `7286fb8fb748f99164b075c8037db77f79ad513e5970e0d046cca13f763b4498` stayed negative. The run also falsified v3 delayed-relevance identifiability: turn 3 repeats turn-1's value and Qwen persists it in an 8k branch hypothesis, so terminal success can bypass raw recall even after the event leaves the recent-eight window. V3 bytes must remain frozen as historical evidence but its 240-cell causal main effect is withdrawn; a successor preregistration must exceed bounded working-state capacity, use a non-repeating terminal cue, and separately score task correctness, raw recall, next-request exposure, and structured consumption. Recall replay itself was hardened by sorting content-addressed call registrations by typed index and using sorted `math.fsum` scores; cross-process tests with four `PYTHONHASHSEED` values now agree. Task `271.3` remains unchecked; no memory benefit, formal identity, scientific result, innovation or publication claim is established._
    - _Progress (2026-08-10, successor v1 protocol implementation before any result): added a separate result-blind delayed-memory successor without changing historical v3 bytes. It defines 5 synthetic domains × 28 independent scenarios = 140 scenario blocks, four within-scenario randomized arms = 560 cells, and 13 turns: 192 high-entropy address/value facts in turns 1—3, eight neutral turns, the query address first selected/released at turn 12, and scoring at turn 13. A frozen 2,048-byte non-sovereign working-state budget applies equally to every arm while permitting any model-chosen compression; only an incomplete projection, overflow, or post-observation truncation is disqualifying, so rigor does not suppress legitimate working-memory innovation. Public commitments are nonce-hardened and contain no direct payload hash, seed, oracle, queried address, expected value, or arm; stimuli/oracles/assignments remain runner-private. The primary A4−A3 analysis uses paired two-sided exact McNemar at alpha .05 with a decision-based .25 SESOI, missing/failed/invalid cells scored zero, no dropped pairs, and actual-use receipts required for every A4-only win before a memory-benefit interpretation. A dense discordance grid from .25 to 1.00 has minimum exact power `0.809086452293` at 1.00. Current hashes are bundle `d6285932929e569a81434ec26641cb9b53feb86e37de01062ce9ed2fbffc906a` and public preregistration `d6ef98c921a4edd15282d726e03353a5e5d8ceab63f29e5c902060817aab86e7`. Twelve focused adversarial tests and 137 linked adaptive-loop/transport/recall tests pass with focused Ruff/Mypy. The bundle, release helper and fragment audit explicitly deny independent secrecy, complete prompt projection, formal execution and result claims. Task `271.3` remains unchecked until a 13-turn public adapter, complete bounded-state projector, external per-turn release service, successor formal runner, fresh secret-seeded pilot, all 560 cells and blind analysis exist; no successor model call or scientific result has been produced._
    - _Verify: mocked adversarial tests for arbitrary operator order, zero Skill, unavailable capability feedback, raw tamper, failed promotion recovery, verifier rejection, temporary-Agent expiry, and policy-mutation denial; one opt-in live Qwen run with at least two autonomous turns; a budget-matched evaluation against fixed pipeline and linear self-loop; focused pytest/Ruff/Mypy, broad regression, Agent/Problem logs, and a focused commit._

- [ ] 272. Add a provider- and topic-neutral Mechanism Research Cycle
  - [ ] 272.1 Freeze the evidence-backed architecture and domain firewall
    - Cross-check autonomous-science, mechanism-discovery, causal-intervention, reliability, provenance, and Open Science primary sources. Treat OPHIS as a team research release with public result snapshots, not as an independently reproduced or peer-reviewed fact.
    - Freeze Observation → Problem → competing Hypotheses → discriminating Intervention → independent Evaluation. Keep speed-up optional and preserve supported, contradicted, inconclusive, and honest negative results.
    - Prohibit every problem-specific object, field, metric, threshold, query, null, baseline, adapter ID, prompt, and method token from the kernel. No single task may establish architecture validity.
    - _Verify: Research/Execution plans and the Obsidian evidence note agree; URLs and source-status labels are checked; Qwen context/output/reasoning limits come from the official page; no problem-specific production term appears in the proposed API; Agent/Problem logs and a focused commit exist._
    - _Progress (2026-08-14): the evidence matrix, source-status boundaries, Mechanism Research Cycle, claim-state machine, Open Science mapping, and hard topic firewall are frozen in both plans and the Vault note. OPHIS is recorded as a team release with public snapshots, not an independently reproduced paper. The item remains unchecked because the shared worktree cannot yield the required focused commit without mixing unrelated in-flight changes._
  - [ ] 272.2 Add local scientific-cycle contracts and Knowledge Graph projection
    - Add strict content-addressed Observation, Problem, Hypothesis, ScientificIntervention, Evaluation, assessment, provenance-binding, and snapshot contracts. Permit incomplete prefixes while rejecting dangling, wrong-stage, overlapping-role, duplicate, and tampered references.
    - Project only typed knowledge nodes and grounds/motivates/tests/assesses edges. External hashes are declaration bindings, not proof that an artifact exists or is scientifically valid.
    - _Verify: abstract synthetic fixtures cover prefix/full/branch-merge cycles, deterministic ordering/hash/schema round-trip, nested tamper, all invalid-reference classes, stage-specific missing fields, assessment evidence rules, and four-plane isolation; focused pytest/Ruff/Mypy pass; no model, experiment, workflow migration, paper, or publication action occurs._
    - _Progress (2026-08-14): the provider/topic-neutral local contract and declared-only Knowledge projection are implemented. Test-first and adversarial review closed projection-tamper, false-verification, impossible-lineage, and cross-plane leakage defects; 39 focused and 179 kernel tests pass, with focused Ruff/Mypy/py_compile clean and no problem-specific term in the new production/test/note files. This item remains unchecked until the required focused commit can be created safely._
  - [ ] 272.3 Add a fail-closed read-only validation bridge
    - Resolve lifecycle bindings against verified ProvenanceBundle, HarnessSpec/EpisodePackage, LoopSpec/LoopRunSnapshot, and EvaluationReport objects without writing or repairing them.
    - Permit supported/contradicted only with valid claim trace, author association, evidence validation, frozen execution identity, independent scientific-validity PASS, results, and uncertainty; otherwise refuse or remain inconclusive.
    - _Verify: wrong hash/type, missing association, dangling evidence, invalid grader/report, incomplete failure, mutation attempt, and historical-artifact upgrade all fail closed; verified negative and inconclusive remain distinct from execution failure and promotion._
  - [ ] 272.4 Shadow-adopt across unrelated verticals and run a frozen architecture benchmark
    - Keep old workflows authoritative while shadow-writing at least two unrelated verticals. Compare execution-only, LLM+Internet, LLM+history, mechanism-only, and hybrid arms under identical call/experiment/compute/time budgets; ablate independent verification, discriminating intervention, lineage, and rollback.
    - _Verify: all-attempt denominator, valid-run rate, hypothesis calibration, competing-hypothesis discrimination, falsification, uncertainty, replication, transfer, post-peak regression, evidence coverage, provenance completeness, cost, human meta-selection, and failures are reported; no task-specific kernel change or holdout reuse is allowed._
  - [ ] 272.5 Connect validated cycles to claim state, Vault, Open Science, and publication gates
    - Advance claims only through idea → literature-grounded → executable → observed → replicated → independently-validated → publication-eligible. Project versioned cycles into Obsidian and existing PROV/RO-Crate exports without overwriting registrations.
    - _Verify: independent replication/external validation, citation and numeric/figure/log consistency, statistical and deviation audit, license/privacy/ethics, and human submission approval are conjunctive; PDF generation, self-review, execution success, or a best run cannot promote a claim._

- [ ] 273. Deliver the Direction A-1 prize-sprint research-plan loop
  - This contest task pauses but does not discard Task 272. It ends at an evidence-backed research plan with a real preliminary signal, feedback revision, independent review, and a Chinese PDF of at most 20 pages. Formal experiments, a result paper, publication eligibility, and autonomous submission are out of scope.
  - [ ] 273.1 Recover and freeze the interrupted evidence lineage
    - Re-derive the old attempt's completed stages, literature denominator, pilot facts, failed plan response, and missing render/review stages. Preserve it byte-for-byte as historical evidence; do not label the rejected draft as a final plan or reuse its pilot under new upstream hashes.
    - _Verify: a factual recovery note identifies stages 1–9 as complete, 10 as rejected, 11–12 as absent; the pilot is described as an exploratory signal with mixed interval evidence rather than a publishable result._
  - [ ] 273.2 Add provider-neutral source query compilation and semantic planning coverage
    - Keep model-authored logical queries, compile them at the arXiv/OpenAlex boundary, and bind compiler version, exact executed query, fetch receipt, and logical lineage. Direct, mechanism/null, and counterevidence queries must reuse the same model-authored object group; every counted role match must have the corresponding targeted-query lineage. Before planning, require direct core ≥2, method foundation ≥1, mechanism/null ≥1, counterevidence ≥1, no off-topic padding, and no method-transfer majority.
    - Recompute coverage after finalist withdrawal/retraction replacement and let exact-metadata duplicate work families consume only one planning slot. Keep citation, venue, DOI, and source corroboration as within-layer quality signals. Do not add any problem-specific production term, whitelist, metric, threshold, null, or adapter rule.
    - _Verify: adversarial unit/integration tests cover source syntax, v1 replay/v2 tamper, role ambiguity, off-topic high-citation noise, coverage deficit, context budget, withdrawal replacement, resume re-derivation, and exact receipt hashes; focused pytest/Ruff/Mypy/py_compile and one opt-in real arXiv/OpenAlex smoke pass._
  - [ ] 273.3 Run one fresh complete twelve-stage Qwen plan loop
    - Start from a new empty root with frozen question PDF, code, provider config, Skills, source set, and required real preexperiment. Do not copy the old pilot or continue a v1 protocol root.
    - _Verify: batch completed=1/failed=0; literature protocol and artifacts are v2; coverage passes with 5–10 real references; all twelve stage receipts exist; the fresh pilot binds this run; model/fetch attempts and failures are fully counted._
  - [ ] 273.4 Audit and deliver the competition research plan
    - Check that preliminary feedback materially narrows or changes the next plan; all observed numbers retain their uncertainty and multiple-testing meaning; unexecuted work is conditional; the independent scientific review has no major blocker; the rendered Chinese PDF is extractable and 1–20 pages.
    - _Verify: final JSON/Markdown/PDF, coverage receipt, pilot metrics, independent review, inventory, and delivery report agree; `formal_experiment_executed=false`, `paper_claimed=false`, and no publication/submission authorization is asserted._
