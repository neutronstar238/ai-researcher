...
- [ ] 269. Run a fresh preregistered lineage targeting the diagnosed zero-term selection
  ...
  - [ ] 269.4 Execute the gated pilot, self-revision, baseline, and full stages
    ...
    - _Blocked-by: the frozen pilot breadth cannot be satisfied on the narrowed panel and cannot be rewritten in place. The system reached `declare_frozen_pilot_breadth_unsatisfiable_and_require_new_preregistration` in all THREE independent live runs with bounded reasoning enabled (`1587`, `1349`, `1144` real reasoning tokens), with `guard_accepted=true`, `edits_frozen_budget_parameter=false`, and `contaminates_finalist_selection=false` every time; only the justification wording varied. A new preregistration is therefore required, and that is beyond the explicit scope of the `268.3` agent-signed approval, which authorizes opening a lineage but NOT changing a frozen budget parameter. `_stage_shape` fails closed so the contradiction cannot be crossed by accident. Zero candidate budget was spent reaching this point._
