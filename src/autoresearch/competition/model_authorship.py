...
def record_model_authorship_receipt(
    *,
    ...
    if path.is_file():
        ...
        if (
            existing.interaction_id == receipt.interaction_id
            and existing.response_sha256 == receipt.response_sha256
            and existing.parsed_payload_sha256 == receipt.parsed_payload_sha256
        ):
            # The model output bytes are immutable and identical; only replay-time
            # context metadata drifted.  The existing receipt stays authoritative.
            return existing
        immutable_fields = {
            "created_at",
            "receipt_hash",
