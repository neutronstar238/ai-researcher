...
- Empirical-gain claims require a positive baseline-vs-candidate method-effect delta in the file-backed innovation evidence. Neutral or negative method-candidate deltas must remain visible and block gain claims.
...
- 经验增益声明必须在文件支撑的创新证据中包含正向 baseline-vs-candidate 方法效果 delta；中性或负向 delta 必须保留并阻断增益声明。
