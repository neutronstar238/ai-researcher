...
function statusDot(status, { pulse = false } = {}) {
  return pulse ? '<span class="pulse"></span>' : "";
}
...
function setBusy(buttonId, busy, busyLabel) {
  ...
  if (busy) {
    ...
    button.innerHTML = `<svg viewBox="0 0 20 20" width="16" height="16" class="spin" aria-hidden="true"><path d="M10 2.5a7.5 7.5 0 0 1 7.5 7.5" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" fill="none"/></svg>${busyLabel || "处理中…"}`;
  } else {
...
async function refreshHealth() {
  ...
  try {
    ...
    if (health.status === "ok") {
      ...
      pill.innerHTML = '<i class="dot"></i><span>服务正常</span>';
    } else {
...
function showHero() {
  ...
  window.setTimeout(() => $("direction").focus(), 0);
}
...
$("single-form").addEventListener("submit", async (event) => {
  ...
  if (!direction) { showAlert("single-error", "请输入题目或研究方向"); return; }
  hideAlert("single-error");
  setBusy("single-submit", true, "创建中…");
  try {
    const result = await api("/api/runs", {
      method: "POST",
      body: JSON.stringify({ direction, dry_run: $("single-dry").checked }),
    });
    toast("运行已创建");
    $("direction").value = "";
    ...
    showAlert("single-error", error.message || "创建失败，请检查服务状态");
    toast(error.message, true);
  } finally {
    setBusy("single-submit", false);
  }
});

$("batch-form").addEventListener("submit", async (event) => {
  event.preventDefault();
  hideAlert("batch-error");
  hideAlert("batch-ok");
  const includeQuestionIds = $("batch-ids").value.split(",")
    .map((item) => Number(item.trim()))
    .filter((item) => Number.isInteger(item) && item > 0);
  setBusy("batch-submit", true, "解析中…");
  try {
    const result = await api("/api/batches", {
      method: "POST",
      body: JSON.stringify({
        question_pdf: $("question-pdf").value.trim(),
    ...
    showAlert("batch-ok", `已创建 ${result.batch_id}：共 ${result.question_count} 个问题（${$("batch-dry").checked ? "Dry-run 仅解析" : "正式批量运行"}）`);
    toast(`批量任务：${result.question_count} 个问题`);
    await refreshBatches();
  } catch (error) {
    showAlert("batch-error", error.message || "创建失败，请检查 PDF 路径与服务状态");
    toast(error.message, true);
  } finally {
    setBusy("batch-submit", false);
  }
});
...
document.querySelectorAll("[data-example]").forEach((chip) => {
  chip.addEventListener("click", () => {
    $("direction").value = chip.dataset.example;
    $("direction").focus();
  });
});
...
$("btn-new-run").addEventListener("click", () => {
  if ($("detail").hidden) {
    // Already on the hero: focus the input.
    $("direction").focus();
  } else {
    showHero();
  }
